...
- [x] **RESOLVED-BY-RETIREMENT 2026-06-29 (Rachel 🕊️, store item `e62d5edc`)**: the `cascade_heartbeat_scheduler.py` daemon was DELETED (milestone I8 + USER BROADCAST `ddaa2882` waiver — liveness is now the Arbiter `:8001` + per-session Stop-hook, no cascade daemon). The tmux-injection race below can no longer fire because the racing code no longer exists. Deletion commit: `46d72786`. No fix shipped (no surviving consumer); the bug retired with the script.
