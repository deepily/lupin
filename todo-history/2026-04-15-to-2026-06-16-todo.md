# TODO Archive — 2026-04-15 → 2026-06-16 (horizon sweep 2026-07-11)

**Archived**: 2026-07-11 (Session 372f9dc9, Mr. Radio 🦉; task-store item 2a190fa2)
**Method**: HORIZON sweep at the 2026-06-25 boundary — Rick-sanctioned hygiene (flagged in the 2026-07-06 STATUS and 2026-07-11 DECISIONS LOG entries: "TODO.md horizon archive … run archive early next session"). Sections dated on/after 2026-06-25, plus the structural `Pending Decisions` / `Pending` queues, remain in TODO.md.
**Contents**: 98 sections, verbatim (pre-sweep TODO.md lines 138-505 + 517-2265).

> **⚠️ STALE-PENDING NOTICE**: 359 still-open `[ ]` bullets ride along in these sections — they were swept by AGE (horizon), not by item-level disposition. Post-cutover (2026-06-17) the unified task-store is the sole liveness source for owed work; anything here that never became a store item is not-yet-owed backlog. Item-level disposition review remains open — see the stale-pending pointer in TODO.md § 📦 Archived.
...
- 2026-07-01 (evening) — branch/worktree debris cleanup → **ALL TIERS + BOTH PRUNES APPROVED (Rick typed "Approved" ~20:11)** and EXECUTED: 38 branches deleted (24 `-d` belt-checked + 14 `-D` patch-id/supersession-proven, double-witnessed by Krishna's audit + Tiberius' independent pass), 2 dead TFE worktrees pruned, 4 stale remote refs pruned; 61→23 local branches. KEPT: 14 historical release lines (Rick retention default), 6 UNKNOWN (owner triage owed), wip/main/lane-c. Proposal + execution receipts: `src/rnd/v0.1.9/2026.07.01-git-branch-worktree-cleanup-proposal.md` §7a. Why: provably lossless (every deletion ancestry-, patch-id-, or store-receipt-backed).
...
- [ ] [LUPIN] **Test-Suite Job — Phase 3 (conftest.py addoption)** — Consolidate `smoke_direct` into unified `smoke` runner via pytest's `pytest_addoption` hooks. ~30 min work. Not blocking. Plan: `src/rnd/v0.1.6/2026.04.05-test-suite-agentic-job-comprehensive-expansion.md` Phase 3.
