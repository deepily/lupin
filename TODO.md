...
- [ ] 🔴 **THE COMMIT-SCOPE GUARD CANNOT PARSE A HEREDOC, SO IT PASSED A COMMIT THROUGH UNREVIEWED AND SAID NOTHING** (Rachel 🕊️, 2026-08-30). `ef8ebcef` went in with a heredoc commit message; the guard could not read its quoting and **let it through with no output at all**, so I checked the scope by hand afterwards. ⇒ **Use `git commit -F <file>`** — a message file has no quoting for the guard to lose — **and verify scope yourself with `git show --name-only <sha>` either way.** 🔴 The durable part is not the heredoc: **a guard that cannot parse its input PASSES, and a silent pass is byte-identical to a clean review.** Same shape as *"THE COVERAGE GATE CANNOT FAIL ON A RED TIER"* above and CLAUDE.md's *"a gate that cannot fail is precisely the thing you cannot prove by watching it pass"* — this is a third instance, in the commit path, where a peer's dirty file is one bad message away from riding in under your name. ⚠️ Nobody has measured **which** message shapes the guard cannot parse; only that this one defeated it.
