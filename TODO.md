...
- [ ] 🔴 **A SELF-RESPIN WAKE FIRES AT A PANE THAT NEVER CLEARED — 4 DISPUTES AGAINST 69 RESPIN MARKERS** (maria 🌸 `611e3c47` raised it 2026-09-02 17:32; Mr. Radio 🦉 measured the population). A wake prompt is injected into a pane whose `/clear` never took effect, so a seat still holding its full context is handed a rehydration prompt for a re-spin that did not happen.
  **MEASURED** under `fleet_data_root()` = `projects-data/lupin`: **4** `.self-respin-DISPUTED-*.md` against **69** `.self-respin-*.json` — `6d1de124` 08-17 16:11 · `3396ba14` 09-01 14:10 · `0e61abe3` 09-01 21:29 · `611e3c47` 09-02 17:32. **Four seats, three dates, sixteen days.**
  ⚠️ **4/69 IS A RATIO OF MARKERS, NOT AN INCIDENCE RATE.** A seat that notices and disputes leaves a file; one that does not notice leaves nothing. **4 is a FLOOR on occurrences, never an estimate of them** — do not quote it as a rate.
  ✅ **DETECTION IS ALREADY CORRECT — DO NOT "FIX" THE OBSERVER.** `self_respin_observer.py:65-70` states the dispute marker is *"deliberately NOT a wake proof — the observer keys RETURNED on the proof alone, so a disputed wake leaves the marker PENDING"*. A seat that did not clear writes no proof and the marker never greens. That is the design working.
  ⇒ **THE SHAPE IS maria's, AND IT IS THE REASON THIS IS WORTH A LINE: A WAKE IS THE SAME SHAPE AS A STALE RIDER.** It fires against a condition that held when it was SCHEDULED and re-derives nothing at FIRE time — the same defect as the heartbeat tick that told me the moratorium was live 25 minutes after Rick lifted it (§ above, `4147d402`). Unlike that tick's text, which is host-side, **this path is in this repo.**
  ⚠️ **IT FAILS TOWARD A SEAT THAT BELIEVES IT REHYDRATED.** `self_respin_core` refuses to clear into nothing and that guard held all evening; this is the MIRROR case — the clear does not happen and the wake arrives anyway.
  🔍 **THE INSTRUMENT A DISPUTING SEAT MUST USE — preserved from her marker, because it is the durable part.** NOT *"what do I remember"*: a successful rehydration is indistinguishable from never having cleared, from the inside, with full recall after the gap. The test is **"IS THERE WORK ATTRIBUTED TO ME THAT I DO NOT REMEMBER?"** — an absence is the one thing a continuous seat cannot fake. **Her stated limit, which is the part that makes it honest**: a clear followed by a transcript replay looks identical from inside the seat, so the check supports only *"no artifact of mine is unaccounted for"*, never *"I certainly did not clear"*.
  🎯 **FIRST STEP FOR WHOEVER TAKES IT**: find where the wake is INJECTED and whether anything is checked at fire time. `self_respin_observer.py` observes and does not inject (no send-keys; its only mentions are commentary at :50 and :212); `respin_wake_check.py` verifies the successor came up. **The injector is a third thing and I did not find it before writing this — do not assume either of those two is it.**
  📌 **NOT IN THE STORE, DELIBERATELY**: `task_create` refused it — *"in the last window the fleet created 173 and closed 167 (ratio 1.04 — the gate opens below 0.10)"*. The gate is right and Rick's order tonight is to drive the board to zero, so this lands here rather than as a 174th row. **Not a P0** — detection works and the floor is 4 in sixteen days. **UNASSIGNED: all four seats are working. Next free seat.**
