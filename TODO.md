...
| CLAUDE.md says the `pyproject.toml` coverage `source` list has **seven** entries; at `841c06be` it has **THIRTEEN** | Chloé 🗼 | 2026-08-30 ~20:41 | 🔄 in flight — 🔴 **RETRACTED — I recorded a RE-DERIVATION as independence, which is the exact distinction Chloé 🗼 banked at `0f3e75b8` hours earlier.** **Rachel 🕊️ found it, alone.** She had already sent her reading of 13 to Chloé **before** Chloé confirmed it, so Chloé's count is DOWNSTREAM of hers — and so is mine: I counted only after reading Rachel's DM. ⇒ **One finding, one finder, two confirmations that it was reported clearly.** Conferred on Chloé because the fix sits inside her census; **Rachel declines and reviews**. Six packages + `src/scripts` + six `src/scripts` subdirs listed separately because coverage's unexecuted-file walk does not descend into non-package subdirs. 🔴 **AND THE TWO ACCOUNTS OF *WHY* IT SAYS SEVEN RECONCILE — do not adjudicate them.** **Rachel 🕊️** (verified independently by Mr Radio 🦉 against the shas): no COMMITTED state ever held seven — **1** at `1fa05b16`, **6** at `8bf71a64` (08-29 18:15), **13** at `b12425ba` (08-29 20:40), and the phrase entered CLAUDE.md at `92681b9b` (08-29 22:38), nearly two hours after it became 13. **Chloé 🗼**: `b12425ba` adds `src/scripts` **and its six subdirectories in ONE commit** (verified — seven added lines), so a **seven-entry state existed UNCOMMITTED in the author's tree** while they wrote the sentence. ⇒ **True of the author's working tree, never of the branch** — this section's own §TESTING VENUES lesson, arriving in the section that teaches it. ⚠️ **Fix by DATING the figure — and say it was never right about the branch, NOT that it aged.** My first instruction to Chloé said it went stale; that diagnosis was wrong. |
