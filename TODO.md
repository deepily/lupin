...
- [ ] **Pocholo 📣 owes** — does the narrow stamp fix REPAIR the 8 broken seats, or only prevent the 9th? Two different outcomes, and the row reads as closed either way.
...
- [ ] **`post-game.md` §5.6 asserts a doc-viewer capability that does not exist** — the `io/post-games/` prefix is not in `.docview.yml`, measured at the whitelist gate. Owner is whoever holds the workflow docs, not this repo.
...
- [ ] **A recheck step in the reap process** (Rachel 🕊️, found while fixing `48b5f19e`) — surfaced by DM, held here under the moratorium rather than filed. Detail owed from her.
