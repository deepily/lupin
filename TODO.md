...
Last updated: 2026-04-10 (Session 85b05d1d — E2E result count mismatch root-cause fix)

## Pending

- [ ] [LUPIN] **#1 notifications.js auth refresh — authedFetch helper + 16 non-compliant sites** — Per plan doc `src/rnd/v0.1.6/2026.04.10-notifications-js-auth-token-refresh-audit.md`. Add `authedFetch(url, options)` helper next to `ensureValidToken()` at ~L908, migrate 16 non-compliant fetch sites first, then 9 compliant ones for uniformity. Verify: click activity log, delete/retry buttons, job history pagination — no 401s after idle. Lupin parent only.
- [ ] [LUPIN] **#2 Test report filename UTC → America/New_York with -EST/-EDT suffix** — Fix `src/cosa/agents/test_suite/job.py:338` `datetime.now().strftime(...)` to use America/New_York timezone and append `-EST`/`-EDT` suffix per project convention. Example: `2026.04.10-at-10:53-EDT-e2e-results.md` instead of `2026.04.10-at-14:53-e2e-results.md`. CoSA submodule — edit only, user commits in CoSA session.
- [ ] [LUPIN] **#3 Move runtime artifacts out of src tree** — Relocate `src/tests/e2e_ui/__snapshots__/` and `src/tests/e2e_ui/snapshot_failures/` to `lupin/io/test-suite/` (user backs up `io/` externally, gitignore non-blocker). Update `pytest.ini:24-25` (`playwright_visual_snapshots_path`, `playwright_visual_snapshot_failures_path`) and hand-move existing PNGs. Re-run `-k visual` to verify path change.
- [ ] [LUPIN] **#4 Visual regression cold/warm baseline drift** — 12 visual tests pass in `-k visual` surgical runs but fail with subpixel font anti-aliasing differences when run as part of the full 335-test e2e suite. Hypothesis: Chromium font/render cache state differs between cold (isolated run) and warm (after 323 prior tests). Options: (a) re-baseline inside full e2e via one-off `--update-snapshots` full run, (b) raise the 0.1 pixelmatch threshold, (c) run visual regression as standalone scheduled suite, (d) order visual regression FIRST in e2e suite via pytest-ordering plugin. Needs own plan.
- [ ] [LUPIN] **#5 Research 5-6 dead/paused jobs stuck in todo queue** — User reports 5-6 dead or paused jobs visible in todo queue that they did not create. Investigate ownership, stuck-state reason, cleanup, whether they're e2e test leftovers or paused scheduled jobs whose `scheduled_at` never fired.
