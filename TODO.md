...
Last updated: 2026-04-06 (Session 93c49ccb — Haiku automated testing default + Phase D follow-ups)
...
- [ ] [LUPIN] **Phase D re-run through test-suite endpoint (AFTER all follow-ups)**. First run was via rogue bg process, not the proper `POST /api/test-suite/submit` path. Re-run with Haiku to validate, then compare against Opus baseline. Also: revise cost cap $2→$5, capture golden baseline, investigate UI queue visibility for ts-* jobs.
