...
Last updated: 2026-04-10 (Session 85b05d1d — Lunch-time autonomous run through #0-#4)

## Pending

- [ ] [LUPIN] **#1-Phase 4 (DEFERRED): migrate remaining 13 compliant fetch sites to authedFetch for uniformity** — Session 85b05d1d implemented Phase 2+3 (helper + 12 non-compliant migrations, commit e68d827). Phase 4 is pure cleanup with no user benefit; can be picked up opportunistically. Plan doc: `src/rnd/v0.1.6/2026.04.10-notifications-js-auth-token-refresh-audit.md`
- [ ] [LUPIN] **#3-Followup: sdla restart lupin-rest to propagate new pytest.ini paths into container** — Session 85b05d1d commit 824f314 relocated visual regression artifacts to `io/test-suite/visual-baselines/` and updated `pytest.ini`. Host-side test (`-k visual`) passes 12/12 with the new paths. The container's bind-mounted `pytest.ini` still points to the OLD inode because my host Edit tool atomically rewrote the file; the bind mount kept its original inode reference. Next `sdla restart` picks up the new inode automatically. Until restart, scheduled test_suite runs (which subprocess-Popen pytest from INSIDE the container) will fail to find baselines. Host-side `./src/scripts/run-e2e-ui-tests.sh` is unaffected.
- [ ] [LUPIN] **#5 Research 5-6 dead/paused jobs stuck in todo queue** — User reports 5-6 dead or paused jobs visible in todo queue that they did not create. Can't query during in-flight e2e (server hot-swapped to Testing config, creds fail). Code-level investigation: queue_consumer waits on paused/future-scheduled jobs indefinitely; paused jobs require explicit resume; scheduled jobs with future timestamps sit until fire time. User's visible stuck jobs are most likely either paused-never-resumed or far-future-scheduled. Post-e2e queries needed to confirm.
