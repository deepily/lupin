...
Last updated: 2026-04-05 (Session 389 — Phase D scheduled + test-suite expansion)
...
- [ ] [LUPIN] **Test-Suite Job — Phase 3 (conftest.py addoption) — DEFERRED to 2026-04-06**. Consolidate `smoke_direct` into unified `smoke` runner via pytest's `pytest_addoption` hooks (--auto-proxy, --cost-cap-usd, --group, etc.). ~30 min work. Not blocking anything. Plan: `src/rnd/2026.04.05-test-suite-agentic-job-comprehensive-expansion.md` Phase 3.
- [ ] [LUPIN] **Test-Suite Job — Persist to DB** (follow-up). TestSuiteJob isn't `AgenticJobBase`, so jobs aren't persisted. Server hot-reload loses scheduled test jobs. Add persistence or extend `is_agentic_job_type` in `job_persistence.py` to include `test_suite`. Discovered Session 389 when Phase D job was lost on server reload.
...
- [ ] [LUPIN] Presentation Generator Phase D verification: Live E2E with real Claude API calls + voice gate interaction. **Session 389: automated + scheduled for 18:30 EDT 2026-04-05 (ts-3d0dddba).** Old 17-step manual checklist SUPERSEDED by `test_presentation_live_smoke.py` + `presentation-gates.json` scripted proxy. Scheduled via `/api/test-suite/submit` with new `smoke_direct` suite type. Plan: `~/.claude/plans/toasty-soaring-perlis.md`. Post-run: capture golden baseline, analyze results.
...
- [ ] [LUPIN] CoSA commit pending: Session 383 — `local_embedding_engine.py` (added `local_files_only=True` to SentenceTransformer)
- [ ] [LUPIN] CoSA commit pending: Session 382b — `configuration_manager.py` (visual grouping stem split: `"_"` → whitespace)
