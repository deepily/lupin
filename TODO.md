...
Last updated: 2026-04-06 (Session 93c49ccb — Haiku automated default + Phase D Haiku run: $0.06, 0 slides)
...
- [ ] [LUPIN] **Docker: Mount ~/.lupin/ as volume for test credentials**. Quick-fixed via `docker cp` in Session 93c49ccb. Permanent fix: add `~/.lupin:/root/.lupin:ro` volume mount to `docker/lupin/docker-compose.yml` so test-env.sh is available inside the container without manual copy. Root cause: server runs in Docker with `$HOME=/root`, but credentials file only exists on host.
...
- [ ] [LUPIN] **Presentation Generator — Haiku as automated testing default**. Session 93c49ccb. Add `automated_content_model` INI key (claude-haiku-4-5-20251001), Haiku pricing to CostEstimate, model override in smoke test. Cost: ~$0.13/run vs $2.43 Opus (95% savings). Validate pipeline completes with Haiku, compare output structure. Plan: `~/.claude/plans/federated-snacking-rabbit.md`.
...
- [ ] [LUPIN] **Presentation Generator — Haiku quality investigation**. Phase D Haiku run produced 0 slides despite pipeline completion. Check YAML artifact `pres-bd87e234` to diagnose. Options: (1) tune prompts for Haiku, (2) try Sonnet (~$0.49/run, 80% savings) as middle ground, (3) accept Haiku for structural regression testing only (pipeline completes = pass).
- [ ] [LUPIN] **Smoke test timeout too short for test-suite path**. `DEFAULT_TIMEOUT=600` but test-suite wrapper adds scheduling + startup overhead. Phase D Haiku job took 192s but the test-suite wrapper timed out at 607s because the proxy + pipeline didn't start until ~400s in. Increase to 900s or make configurable via `--timeout` CLI arg.
