...
- [ ] [LUPIN] **Task-store v0.3 cold /plan-review (pre-Phase-1 gate)** — Rick ruled all four forks 2026-06-11 night (F1 PostgreSQL-now · F2 extend-:7999 WITH async-handlers debt STAFFED as Phase-1 prerequisite · F3 store-canonical/TODO-narrative · F4 managers-first + don't-lose-the-widening rider); design v0.3 DESIGN-RULED at PIP `src/rnd/2026.06.11-unified-task-store-design.md` §3.1. Per Rick: ONE cold lightweight review (3 serial passes: usability/reuse → fitness → ownership-audit) by a FRESH reviewer — NOT Tiberius (reviewed v0.1), NOT anyone v0.1-adjacent. Tiberius spawns the cold reviewer off-peak (post-midnight or morning; deferred 2026-06-11 22:30 EDT after the fleet-wide Max-plan session-limit freeze). Owner: Tiberius (manager seat).

- [ ] [LUPIN] **Task-store Phase-1 build — post-Saturday-cutover, entry gate = async-handlers fix staffed** — Tiberius crew is recorded Phase-1 owner (accepted in v0.1 review, capacity permitting). HARD entry gate per Rick's F2 ruling: the `:7999` async-handlers/HTTP-starvation debt (bug-fix-queue.md; 2 wedges 2026-06-11 + suspected cause of the double worker dead-call wedge) must be STAFFED as the Phase-1 prerequisite. Sequence: Saturday cutover lands → async-debt staffed → Phase-1 build under the standing green+reviewed gate.
