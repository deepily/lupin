...
- [x] ✅ **ANSWERED, and it is the worse of the two: it PREVENTS, it does not HEAL** (Pocholo 📣, measured 2026-08-30 ~21:30 EDT). `51950988`'s own message said so — *"this stops the bleeding; it does NOT heal… the eight erased seats stay erased until each restarts"* — so the answer existed and had not been carried back to this list. Now measured rather than quoted: **9 of 11 live bridges carry `manager_figure_implicit`; 2 do not.** The two are exactly the oldest seats, **María 🌸 (persona assigned 13:47) and Tiffany 💍 (13:45)** — both predating the fix (landed 15:21) and neither restarted since. Every seat allocated after 15:21 has the field. **The population healed by RE-SPINS, not by the fix.**
  🔴 **And they are not stale files — both `cc_pid`s are ALIVE** (29186, 24174, verified in `/proc`). So two live seats will still get a 403 on a blocked mint, and the remedy for those two really is a restart. ⇒ Nothing more to build; the fix is correct and its scope was stated honestly. Someone re-spinning María or Tiffany closes the last two.
