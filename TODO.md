...
- [ ] 🔴 **RETRACTED AND REPLACED — THE GUARD IS NOT SILENT ON THE SHAPES MEASURED, AND “ALLOW ON DOUBT” IS ITS DESIGN RATHER THAN ITS DEFECT** (Rachel 🕊️; challenged by Chloé 🗼 within two minutes of the commit, 2026-08-30). **My first version of this bullet said the guard passed `ef8ebcef` through “with no output at all” and “said nothing”. That is withdrawn.** I never saw that terminal and cannot — it was my predecessor's session — and my memento claims only *unreviewed*, never *silent*. **The word was mine, added tonight, and it is the same failure this book already catalogues: a symptom inherited from somewhere else and restated as something observed.**
  **What the guard actually does, read out of the file rather than inferred.** The give-up path is `_notice_for()`, which prints **`⚠️ Commit scope guard: NOT REVIEWED — <why>`** followed by *“This commit was allowed unexamined”* and the remedy. Its docstring names the ruling that put it there: *“the non-blocking ‘I could not review this one’ (mr radio's ruling, 2026-08-25)”*. The module header states the policy outright — *“ALLOW ON DOUBT, AND SAY SO”* and *“FAIL-OPEN: ANY error → allow”*. ⇒ **The pass is announced by design.** **And it is OBSERVED, not only read** (Chloé 🗼, correcting the first cut of this headline): **five heredoc commits of hers all printed `NOT REVIEWED`.** That is the stronger half of the evidence and it is hers — mine reads the code path, hers watched the notice appear. ⚠️ **Which is exactly why the headline now says ON THE SHAPES MEASURED**: between us we have four probed shapes and five live commits, and **none of them is `ef8ebcef`**. *“The guard is not silent”* is a claim about every shape; what we hold is a claim about ours.
  **And my remedy was narrower than I wrote it.** Measured by driving `_strip_heredoc_bodies` + `_pathspec_of` on four command shapes: a heredoc that writes the message file followed by `git commit -F "$MSG"` **in the same command parses cleanly** (`why=None`) — that is the shape behind `32437750` and `316bcb0f`, so those were examined, not skipped; `git commit -F - <<'EOF'` **gives up**, with *“the command carries a redirection this guard will not try to read”*; plain `-F <file>`, with or without `-- <paths>`, parses. ⇒ The rule is **not** *“a message file has no quoting for the guard to lose”*; it is **do not pipe the message through a redirection into `-F -`**. ⚠️ The stripper's own docstring still narrates the older defect where the `-F <file>` shape went unreviewed (row `292dd3d8`); **that was fixed, and the comment reads like a live warning** — which is how I came to believe it.
  ⇒ **The durable half, and it is smaller than the bullet I first wrote**: the guard allows on doubt on purpose, so **a commit that PASSES is not the same as a commit that was REVIEWED, and the printed notice is the only thing that distinguishes them.** Read it when it appears, and check scope yourself with `git show --name-only <sha>` either way. ⚠️ **Still unverified and now probably unverifiable**: what `ef8ebcef`'s terminal actually printed. Nobody should carry that claim forward in either direction.
