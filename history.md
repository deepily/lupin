...
#### Checkpoint | 2026.05.06 22:00 | Phase 6a code-writing cycle — all 7 phases shipped (8 commits, c8 100%, AC1-AC10d ✓)

**Context**: Resumed after `/clear` via `92-resume-here-phase6a-codegen.md`. Documentation cycle was closed; this checkpoint is the implementation cycle. User authorized end-to-end execution + per-phase checkpoints in a single go-ahead.

**Accomplishments**:

- **Phase 0 (tracking docs)** — `2026.05.06-phase6a-code-execution-plan.md` (NEW, ~410 LOC) serialized per `feedback_phase0_serialization_prominence`; `90-execution-log.md` Phase 6a sub-section seeded with 7-row per-phase progress table + 16-row AC scorecard + side-effect checklist.
- **Phase 1 (server-side dep)** — `multiplexer max meta display bytes = 256000` INI key + splainer entry; NEW endpoint `GET /api/multiplexer/config` authored as `src/cosa/rest/routers/multiplexer_config.py` per the established all-routers-in-CoSA convention; main.py register added; `src/docs/rest-api-reference.md` §24 added. Endpoint smoke-verified via `urllib.request` on :7999. (CoSA file uncommitted on the CoSA side per `feedback_lupin_only_never_cosa` — user handles that commit separately.)
- **Phase 2 (formatDuration)** — Pass 2 F24 satisfied: `formatDuration(startTs, endTs?)` added to `render/time.ts`; 8 new unit tests cover Ns / Nm Ms / Nh Mm / "running for ..." / NaN-Infinity / clock-skew clamp; c8 100% maintained on time.ts.
- **Phase 3 (templates)** — `jobCard.ts` + `jobBucket.ts` authored (267 + 172 LOC); 38 PASS (AC3 24 + AC4 14); c8 100% across both files. F23 (no `data-id-hash` on delete button), F18 (only 4 valid Job.status values, no `interrupted`), F30 (Enter/Space keydown + aria-expanded reflection on bucket header) all enforced via dedicated tests. `safeStringifyMeta` carries cycle-protection via WeakSet visited tracker. Templates use direct DOM ops for the cards container after discovering the `html` tagged-template `ATTR_NAME_REGEX` can't concatenate static strings with interpolations inside attribute values.
- **Phase 4 (renderer)** — `JobsPaneRenderer.ts` (269 LOC) + 23 PASS unit tests (target was ≥18); c8 100%. `shared/types.ts` extended with `hydration_failed` LupinEventType + `HydrationFailedPayload` interface + `jobsRenderer?: string` on `BootCompletePayload.handlers`. Tests 13-18 from design (constructor falsy throw, missing container throw, deferred-promise success/rejection races per F6+F27, mount idempotency throw F26, disabled-delete-button no-op F23) all green; extras 11b/16b/19b added to push c8 to 100%. AC2a `grep -rn hydrateHistory` returns ≥1 match — Phase 5 ban LIFTED.
- **Phase 5 (CSS + page shell + boot)** — `jobs-pane.css` NEW at 324 LOC (AC10b ≤800 ✓); status-run translated to status-running per Job.status enum; status-interrupted DROPPED per F18. `.stylelintrc.json` overrides block enforces F28 layer-2 (`selector-disallowed-list [*, html, body, :root]`) — verified to fire on `body{}` test. `multiplexer.html` populated #jobs-pane structure + added CSS link. `boot.ts` wires `createJobsPaneRenderer` after Phase 5 mount + emits 2 stable AC9 lines (`[multiplexer] notificationsRenderer:mounted` and `[multiplexer] jobsRenderer:mounted`) BEFORE the JSON form + floats `/api/multiplexer/config` fetch. eslint config gets `argsIgnorePattern: "^_"`. Build: `boot.65c779ac946b.js` gz=31484B (AC7 ceiling 60382 → ✓; +1822B vs Phase 5 baseline 29662).
- **Phase 6 (smoke + cross-phase verification on :7999)** — `test_multiplexer_phase6a_smoke.py` (NEW, 5 tests): functional smoke + parameterized exact `data-phase6-pending` count + perf gate (50 jobs <150ms) + AC9 stable-line handshake + AC10d layer-3 CSS-canary (programmatically disables jobs-pane.css and proves it contributes ZERO body-style drift). All 5/5 PASS. Cross-phase :7999 sweep all green: tsc + eslint + Phase 1 calc smoke + WS smoke 50/50 + 470/470 multiplexer unit + AC6 c8 100% on 3 new render files + AC10b 324 LOC + stylelint clean.
- **Phase 7 (E2E test authored; AC11a/AC11b user-pending)** — `test_multiplexer_phase6a_visual.py` (NEW): 5-job fixture spans all 5 buckets with FIXED timestamps + `_STABILIZE_DOM_JS` pinning all `.job-timing` text. Test collects cleanly via `pytest --collect-only`. AC11a Run #1 (baseline) + AC11b Run #2 (regression) require user-confirmed `:8000` `scheduled_at` slots per `feedback_test_server_monopolize_mode` — AI cannot side-door inject. cosa-voice slot-ask returned a transient 503; deferred to user-coordinated session.

**Process notes**: Per `feedback_approved_sequences_execute_end_to_end`, single user go-ahead covered all 7 phases. One mid-flight architecture mistake — initially proposed re-architecting the FastAPI router into parent-Lupin space to avoid the cross-repo commit dance, conflating `feedback_lupin_only_never_cosa` (no git) with file-edit prohibition. User caught it pointedly; reverted to the canonical CoSA-router approach. After the correction every phase landed cleanly with no further redirects.

**Files** (parent-Lupin scope only): 10 new + 12 edited. New TS: `JobsPaneRenderer.ts`, `templates/jobCard.ts`, `templates/jobBucket.ts`. New CSS: `jobs-pane.css`. New tests: 3 unit (`templates_job_card.test.ts`, `templates_job_bucket.test.ts`, `jobs_pane_renderer.test.ts`) + 1 smoke (`test_multiplexer_phase6a_smoke.py`) + 1 E2E (`test_multiplexer_phase6a_visual.py`). New tracking: `2026.05.06-phase6a-code-execution-plan.md`. Edits: `lupin-app.ini`, `lupin-app-splainer.ini`, `main.py`, `rest-api-reference.md`, `render/time.ts`, `time.test.ts`, `render/index.ts`, `shared/types.ts`, `.stylelintrc.json`, `multiplexer.html`, `boot.ts`, `dev-tools.html`, `90-execution-log.md`, `multiplexer/.eslintrc.json`. CoSA-side (NOT committed by parent session): `multiplexer_config.py`.

**Status**: Phase 6a implementation cycle CLOSED on the :7999 side. AC11a/AC11b await user :8000 slot-coordination. Phase 6b (interactive widgets + TTS chrome + delete-button handler) opens next when scheduled.

**Commits**: `362fa5d` (Phases 0+1), `1bf7ce9` (Phase 2), `597455d` (Phase 3), `4c9d207` (Phase 4), `70a29ff` (Phase 5), `ac273e5` (Phase 6), `5cd8b20` (Phase 7), `744b6dd` (tracking-doc backfill).
