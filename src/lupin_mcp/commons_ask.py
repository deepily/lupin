...
def ask_async(
    ...
    """
    Post a question to a topic at large and return immediately (polling-mode).

    Polling contract: caller polls `CommonsStore.read(topic, since=posted_ts)`
    and filters for entries whose `metadata.in_reply_to == question_id` to
    detect answers.

    (The Phase 3 push-mode / inter-session-DM register path was removed in the
    cosa-voice token-reduction full-removal pass, 2026-06-17 — directed peer DMs
    now use the notification-native `dm_send` path. `ask_async` is polling-only.)
