...
# FLEET DATA backup — rows 8758d0b1 / f56fc63b. Instantiated from the canonical
# planning-is-prompting template.
#
# ONE script for the ENTIRE projects-data tree, its OWN destination, completely
# partitioned from the repo backups (Rick's ruling 2026-07-26). Deliberately NOT a
# second source on sync-to-backup.sh: that script is a single SOURCE->DEST pair with
# --delete LIVE, so appending a source there would merge this tree into the LUPIN
# backup destination and delete whatever did not match.
#
# COVERS EVERY REPO'S DATA AT ONCE — the tree is <DEEPILY_DATA_DIR>/<repo>/..., so
# one invocation backs up lupin, planning-is-prompting and every future repo. There
# is no per-project multiplication here.
#
# OWN EXCLUDE FILE, deliberately. rsync-exclude.txt is 99 patterns reasoned about
# against a SOURCE tree and zero against a data tree; its coverage of these files is
# accidental rather than intended, and drifts the first time someone adds a
# source-tree pattern without knowing a data tree reads the same file.
#
# WHAT THIS DOES AND DOES NOT PROTECT: --delete makes this a MIRROR, not an archive.
# It covers DISK FAILURE. It does NOT protect against wrongful deletion — a janitor
# that reaps a hold file will have that deletion propagated here on the next run.
# Versioned snapshots are a named gap, not a promise.
#
# Usage:
...
SOURCE_DIR="${DEEPILY_DATA_DIR:-/mnt/DATA01/include/www.deepily.ai/projects-data}/"
DEST_DIR="/mnt/DATA02/include/www.deepily.ai/projects-data/"
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
EXCLUDE_FILE="$SCRIPT_DIR/conf/rsync-exclude-data.txt"
PROJECT_NAME="Deepily FLEET DATA (projects-data)"
# === CONFIG END ===
