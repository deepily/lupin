...
export function renderTaskLookupBox( opts: TaskLookupBoxOptions ): TaskLookupBoxHandle {
  const tid = "multiplexer-task-lookup";
  const root = document.createElement( "div" );
  root.className = "task-lookup";
  root.setAttribute( "data-testid", tid );

  const input = document.createElement( "input" );
  // "text", NOT "search" (Rick, row 700f0e1d): a search input gets the browser's
  // own ✕, which empties the box but leaves the filter on. The real clear is
  // `clearBtn` below. enterkeyhint keeps the search key on a phone keyboard.
  input.type = "text";
