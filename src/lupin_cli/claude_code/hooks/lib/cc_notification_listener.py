...
class CCNotificationListener( BaseWebSocketListener ):
    ...
    def _inject_exit_conversation_reminder( self ):
        ...
        prompt. Triggered by an `action:exit_conversation_mode` push from
        the conversation-mode router when this session has been displaced
        by another session entering conversation mode.

        The reminder body is generated by hook_common.conv_mode_exit_reminder
        ...
        already flipped to active=false).

        Ensures:
            - Tmux pane receives the reminder followed by Enter
            - Never raises (logging-only on injection failure)
        """
        try:
            from lupin_cli.claude_code.hooks.lib.hook_common import conv_mode_exit_reminder
            reminder = conv_mode_exit_reminder()
        except Exception as e:
            self._log( f"{self.LOG_PREFIX} conv_mode_exit_reminder import/build failed: {e}" )
            return
    ...
    def _inject_via_tmux( self, message_text, wrap=True ):
        ...
        Requires:
            ...
            - wrap is a bool — True (default) applies conv_mode_wrap, False
              injects the text verbatim. Set False when the caller has
              already produced a complete <system-reminder> block (e.g.
              the exit-conversation action handler) that must reach the
