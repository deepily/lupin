...
class TestTTSFractionSlider:
    ...
    def _open( self, page, *, pre_override: float | None = None ):
        """Seed auth on the managed page, optionally pre-seed the override, navigate.

        Uses the pytest-playwright `page` fixture (loop-managed) instead of a
        manual `sync_playwright()` launch, which trips "Sync API inside the
        asyncio loop". add_init_script runs before app boot on goto.
        """
    ...
    def test_slider_has_nine_stops_at_12_5_step( self, page ):
        self._open( page )
    ...
    def test_moving_slider_updates_label_and_persists_override( self, page ):
        self._open( page )
    ...
    def test_stored_override_seeds_slider_on_reload( self, page ):
        self._open( page, pre_override=0.875 )
