...
def test_run_git_success():
    ...
    with patch( "asyncio.create_subprocess_exec", return_value=proc ):
        result = asyncio.get_event_loop().run_until_complete( git_ops._run_git( "rev-parse", "--abbrev-ref", "HEAD" ) )
...
def test_run_git_failure():
    ...
    with patch( "asyncio.create_subprocess_exec", return_value=proc ):
        result = asyncio.get_event_loop().run_until_complete( git_ops._run_git( "status" ) )
...
def test_commit_on_branch_success():
    ...
    with patch( "asyncio.create_subprocess_exec", side_effect=procs ):
        result = asyncio.get_event_loop().run_until_complete( git_ops.commit_on_branch( [ "file1.py", "file2.py" ], "[BFE] Fix bug" ) )
    assert result[ "success" ] is True
...
def test_commit_on_branch_rejects_empty_files():
    git_ops = GitOps( debug=False )
    result = asyncio.get_event_loop().run_until_complete( git_ops.commit_on_branch( [], "test" ) )
    assert result[ "success" ] is False
...
def test_commit_on_branch_add_fails():
    ...
    with patch( "asyncio.create_subprocess_exec", return_value=proc ):
        result = asyncio.get_event_loop().run_until_complete( git_ops.commit_on_branch( [ "missing.py" ], "test" ) )
    assert result[ "success" ] is False
...
def test_create_fix_branch_success():
    ...
    with patch( "asyncio.create_subprocess_exec", return_value=proc ):
        result = asyncio.get_event_loop().run_until_complete( git_ops.create_fix_branch( "fix/2026-04-05-null-pointer" ) )
    assert result[ "success" ] is True
...
def test_create_fix_branch_rejects_empty_slug():
    git_ops = GitOps( debug=False )
    result = asyncio.get_event_loop().run_until_complete( git_ops.create_fix_branch( "" ) )
    assert result[ "success" ] is False
...
def test_commit_and_push_sequence():
    ...
    with patch( "asyncio.create_subprocess_exec", side_effect=procs ):
        result = asyncio.get_event_loop().run_until_complete( git_ops.commit_and_push(
...
def test_create_pr_success():
    ...
    with patch( "asyncio.create_subprocess_exec", return_value=proc ):
        result = asyncio.get_event_loop().run_until_complete( git_ops.create_pr(
...
def test_create_pr_gh_unavailable_degrades():
    ...
    with patch( "shutil.which", return_value=None ):
        result = asyncio.get_event_loop().run_until_complete( git_ops.create_pr( "fix/test", "t", "b" ) )
    assert result[ "success" ] is False
...
def test_get_current_branch_success():
    ...
    proc = _mock_proc( returncode=0, stdout=b"main\n" )
    with patch( "asyncio.create_subprocess_exec", return_value=proc ):
        branch = asyncio.get_event_loop().run_until_complete( git_ops.get_current_branch() )
    assert branch == "main"
...
def test_checkout_branch_success():
    ...
    proc = _mock_proc( returncode=0 )
    with patch( "asyncio.create_subprocess_exec", return_value=proc ):
        result = asyncio.get_event_loop().run_until_complete( git_ops.checkout_branch( "main" ) )
    assert result[ "success" ] is True
    assert result[ "error" ] is None
