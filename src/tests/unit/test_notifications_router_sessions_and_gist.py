...
class TestTheVisibleSenderList:
    ...
    def _personas( self, monkeypatch ):
        ...
        Since row 41da77bb the handler resolves every sender against ONE bridge index
        (`_stamp_sender_personas`) instead of calling the per-sender helpers, so the
        seams patched here are the index build, the index lookup, and the two
        entry-to-badge projections.
        """
        ...
        def _find( index, sid_suffix ):
            assert index == "INDEX-VALUE"
            return ( f"BRIDGE-PATH-{sid_suffix}", { "suffix": sid_suffix } )

        def _voice( entry ):
            if entry is None: return None   # a sender id with no #session suffix
            self.voice_args.append( entry[ 1 ][ "suffix" ] )
            return f"VOICE-FOR-{entry[ 1 ][ 'suffix' ]}"

        def _manager( index, entry ):
            assert index == "INDEX-VALUE"
            if entry is None: return None
            self.manager_args.append( entry[ 1 ][ "suffix" ] )
            return f"MANAGER-FOR-{entry[ 1 ][ 'suffix' ]}"

        monkeypatch.setattr( notif, "_build_live_bridge_index",    _build_index )
        monkeypatch.setattr( notif, "_find_in_bridge_index",       _find )
        monkeypatch.setattr( notif, "_voice_persona_from_bridge",   _voice )
        monkeypatch.setattr( notif, "_manager_persona_from_bridge", _manager )
    ...
    def test_each_sender_carries_both_badges_from_its_own_id( self, harness ):
        harness.repo.returns( "get_sender_last_activities_visible",
                              [ self._activity( "claude.code@lupin.deepily.ai#SENDER-A" ) ] )
    ...
    def test_two_senders_get_their_OWN_badges_not_the_first_ones( self, harness ):
        ...
        harness.repo.returns( "get_sender_last_activities_visible",
                              [ self._activity( "claude.code@lupin.deepily.ai#SENDER-A" ),
                                self._activity( "claude.code@lupin.deepily.ai#SENDER-B" ) ] )
        ...
        assert self.index_builds == 1, "the bridge index was rebuilt per sender"
