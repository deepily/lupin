...
KNOWN_ABSENT = {
    "docker-compose.cloud-gpu.yml": {
        ...
        "weil-nda-drafting-suite"    : "registered 2026-08-31 as the SECOND of the two weil-* repos Rick asked to be viewable, alongside weil-parallel-search. Same standing as that entry and for the same reason: nobody has confirmed it exists on the VM and gcloud auth was expired, so no seat could read that disk. ⚠️ A MOUNT WAS DELIBERATELY NOT ADDED — cloud-gpu binds long-form with create_host_path: false, so a bind whose host path is absent stops the CONTAINER, which is the very failure being fixed that day. Registering the scope is fail-soft (a missing path warns and skips); adding a bind is fail-hard. When the directory is confirmed on the VM the right fix is a MOUNT and this entry must go — the reverse-direction test forces that the moment one lands.",
    },
}
