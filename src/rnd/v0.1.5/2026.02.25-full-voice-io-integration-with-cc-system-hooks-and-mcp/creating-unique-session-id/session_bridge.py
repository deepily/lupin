#!/usr/bin/env python3
"""
Session bridge for CoSA Voice MCP → Claude Code session_id.

Problem:
    MCP servers are spawned BEFORE SessionStart hooks fire.
    Claude Code does NOT expose CLAUDE_SESSION_ID as an env var.
    (See: https://github.com/anthropics/claude-code/issues/17188)
...
Usage in cosa_voice_mcp.py:
    from session_bridge import get_claude_session_id, wait_for_session_id
...
def _find_session_file() -> Optional[Path]:
    """
    Find the session file for the current MCP server's parent process.

    Strategy: Walk up the process tree to find a cc-{pid}.json file.
    The MCP server's parent is Claude Code (or a wrapper), whose PID
    was used by the SessionStart hook to write the session file.
    ...
    # Try own PPID first (MCP server → Claude Code)
    ppid = os.getppid()
    direct = SESSION_DIR / f"cc-{ppid}.json"
    if direct.exists():
        return direct

    # Try grandparent (MCP server → wrapper → Claude Code)
    try:
...
def get_claude_session_id() -> str:
    ...
    Returns:
        str: Session ID (8-char hex fallback, or full UUID from Claude Code)
    """
...
def get_session_metadata() -> dict:
    """
    Get full session metadata (session_id + transcript_path + cwd).

    Returns:
        dict with session_id, transcript_path, cwd, source
    """
    env_id = os.getenv( "CLAUDE_SESSION_ID" )
    if env_id:
        return {
