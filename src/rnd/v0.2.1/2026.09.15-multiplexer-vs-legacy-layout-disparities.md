...
| 5 | **Does the multiplexer hide Job Queues by default?** | **No — show it** (evening). A prior ruling said yes, on a premise now measured false | S12: `DEFAULT_HIDDEN_SECTION_IDS` (`sectionToolbar.ts:83`) drops `"jobs-pane"`, and its annotation must **name the reversal** rather than keep citing "legacy hides it" |
