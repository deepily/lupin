...
⚠️ **`/api/init` is unauthenticated** (`system.py:275-280`, no `Depends(get_current_user)`) and reinitializes the config singleton, able to swap the database connection by query param. Legacy carries no admin gate here. **Escalated to Rick as §6 item 17** (row `e5ebc815`) — a security-posture change on a live endpoint is his call, not a plan's imperative. María 🌸's recommendation: rule it admin-only, consistent with B-3, and specify disabled-while-in-flight.
...
| 2 | Ratify or overrule **all six open questions in [the spec](2026.09.15-operator-state-preservation-spec.md)** — including the persistence idiom for A-1c and A-2 #6 | Mr. Radio 🦉 | **A-1** | 🔴 **open** — *reverted from done.* Revision 3 marked it settled off the spec's own §8, whose heading still reads "María's recommendation — Mr. Radio to ratify or overrule". **A recommendation is not a ratification.** Spec open question **5 is a MEASUREMENT, not a decision** — an implementer runs it against `render/requestChips.ts` |
...
| 7 | Pre-allocate the seven `boot.ts` mount slots in one commit — **Phase B item zero** | **an implementer Mr. Radio 🦉 spawns**; he owns the sequencing | Phase B | open — 🔴 *revision 3 assigned this to Mr. Radio himself. Seven ids and insertion points across `multiplexer.html` and `boot.ts` is prod code, and a Manager does not build* |
| 8 | Decompose A-2 #2 and #3 into named sub-items | Mr. Radio 🦉 | Phase A | open |

| 9 | **Pick the toolbar mechanism for a section that owns two mounts** — the `💬` button must hide the header region as well as the pane, which today's one-`sectionId`-one-element model cannot express (`SectionToolbarRenderer.ts:153-160`; `boot.ts:380` vs `:365`). Cheapest is `sectionIds: string[]` with the persisted key staying the first id | Mr. Radio 🦉 | **A-2 #4** | open |
| 10 | **Rule the `wireSectionCollapse` contract**: extend persisted collapse to all **eight** consumers, or state why Finished Tasks alone diverges (`sectionHeader.ts:12-14`) | Mr. Radio 🦉 | **A-2 #6** | open |
| 11 | **Specify `query_unavailable`'s detection condition and the model-union edit** — it has no producer today (`taskListModel.ts:65`, `TaskListStore.ts:329`), so the copy at `HoldingAreaRenderer.ts:61-87` is only the rendering half | Mr. Radio 🦉 | **A-2 #7** | open |
| 12 | **Rule TTT/TTFA/RTT in or out of the first cut** — Q7's four capture points span the socket and audio pipeline and no Phase B row owns those files. Either add the instrumentation to B-1 naming `AudioStore` and `wireTtsPlayback`, or rule the metrics out and say so | Mr. Radio 🦉 | **B-1** | open |
| 13 | **Rule which session id is `websocket_id`** — the multiplexer keeps two (`transportSessionIds.ts:22-25`) and neither Phase 2 nor this plan says which the wire contract means (`claude_code_queue.py:24-43`) | Mr. Radio 🦉 | **B-2** | open |
| 14 | **Specify the auto-fix checkbox's pre-resolve value** — J20 overwrites it at config load from the INI through the existing `boot.ts:262-266` seam; legacy's markup default is unchecked, and the value shown before the config lands is unstated | Mr. Radio 🦉 | **B-2** | open |
| 15 | **Specify `isAdmin: () => boolean` in the store options, and the refusal semantics** — `NotificationStore` has no path to an admin predicate today, while F3 must refuse a non-admin mode change at the setter (legacy warns and returns) | Mr. Radio 🦉 | **B-3** | open |
| 16 | 📏 **MEASUREMENT, not a decision — read `stores/jobViewFilter.ts` line by line.** Phase 2 F8 checked the live-event filter predicate at name level only and flagged it rather than asserting it. An inherited unverified claim must not reach a builder as fact | **an implementer Mr. Radio 🦉 spawns** | **B-3** | open |
| 17 | **The `/api/init` admin gate** — the endpoint is unauthenticated (`system.py:275-280`) and can swap the database connection at runtime; legacy carries no gate. This is a **security-posture change on a live endpoint**, and the superset ruling covers extras, not restrictions | **Rick** — escalated, row `e5ebc815` | **B-5** | 🔴 **blocked on Rick**, filed with three options and a recommendation; deferred rather than fired because he is AFK |

🔴 **Phase A does not start until §6 is clear** — every row above, not a numbered subset. **Mr. Radio 🦉** holds that gate.
