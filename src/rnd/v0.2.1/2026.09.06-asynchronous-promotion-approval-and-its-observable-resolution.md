...
| **§5.4.1** — the two defects themselves, found by SELF-AUDIT against that attack line | **mine**, and the overclaim being audited was also mine |
| **§5.5** — that a 202 is a false green in the browser clients | **Mr. Radio 🦉**, reported to me; I verified it and established the population |
| **§5.5.2** — that `Record<string, string>` carries the string `"true"`, so the opt-in must be `StrictBool`, AND that my defence sat in the client's type system where `notifications.js` has none | 🔴 **Tiffany 💍, in independent review — hers alone.** She attacked my construction argument at the one point it could fail, and it did not survive |
...
**closed set** — `reason`, `park_reason`, `next_chase_ts` (`:12553-12555`).

### 5.5.2 🔴 AND THAT ARGUMENT WAS VOID — `Record<string, string>` CARRIES `"true"`

**Tiffany 💍's finding, in independent review, and it is hers alone.** She attacked the
construction argument at the one point where it could fail and it did not survive.

**My claim**: *a boolean cannot be placed in a `Record<string, string>`, so a browser can
never opt in.* **True about the type, and irrelevant** — the map carries the **string**
`"true"` happily, and a server doing a truthy check on that field is opted in.

**Measured rather than reasoned, pydantic 2.13.3, one variable:**

| input | `Optional[bool]` | `Optional[StrictBool]` |
|---|---|---|
| `True` | ACCEPTED → `True` | ACCEPTED → `True` |
| `'true'` | 🔴 **ACCEPTED → `True`** | **REJECTED (422)** |
| `'True'` | 🔴 ACCEPTED → `True` | REJECTED (422) |
| `'false'` | 🔴 ACCEPTED → `False` | REJECTED (422) |
| `'1'` / `1` / `'yes'` | 🔴 ACCEPTED → `True` | REJECTED (422) |

⇒ **A plain `bool` COERCES. The opt-in field must be `StrictBool`**, and stage 2's
definition of done includes the arm where `"true"` is **rejected by the real endpoint** —
not by a scratch model like the one that produced this table.

🔴 **THE DEEPER CORRECTION IS WHERE MY ARGUMENT LIVED, NOT MERELY THAT IT WAS WRONG.** I
put the guarantee in the **client's type system**. That is why her second point lands so
hard: **`notifications.js` has no type system at all**, so under my version **layer 2 was
undefended and I had not noticed** — I had written a two-layer denominator and then
proposed a one-layer defence. `StrictBool` **at the server** defends both layers
identically.

⇒ **A guarantee belongs where every caller must pass, never where only one kind of caller
is checked.** Her attack did not just find a hole; it **relocated the argument**.

⚠️ **One pre-existing property in our favour, and it is not mine**: `TaskTransitionIn`
already carries `extra="forbid"`, so **today** an unknown `asynchronous` key is a 422 for
everyone. The window opens only when the field is added — which is exactly when
`StrictBool` must land with it, not after.

⚠️ **This is the same principle as the ask-status allowlist in §3, applied one layer out:
an unknown party gets the safe answer, not the new one.** ⇒ And §5.5.2 is what it looks
like when that principle is asserted in the wrong place.
...
## 11 · What stages 2 and 3 actually landed, and the one thing they did not

**Written after the build, at `1c16dc10`+, by Tiberius 👑 — a re-spin of the seat that
wrote §0–§10.** Every claim below is a receipt from a run in this worktree, not a plan.

### 11.1 They landed TOGETHER, which §8.4 required and Mr. Radio re-confirmed

§8.4 says *"if stage 3 is not ready, stage 2 does not land"*. Mr. Radio confirmed it by DM
at 17:22 EDT. So the 202 and the sweeper are one unit: **the branch never holds a state
in which a 202 is reachable and the suspenders are not.**

### 11.2 The gate split into its two halves, and why that IS the staging requirement

`approval_for_promotion` now holds no logic. It composes two named halves:

| half | what it decides | who calls it |
|---|---|---|
| `promotion_precheck` | credentials + the ask-exempt case — everything decidable WITHOUT asking Rick. Returns a settled `PromotionApproval`, or **None**, which means one thing only: *the ask must fire* | both doors, inside the request |
| `approval_from_the_ask` | the 120-second half | the synchronous door immediately; the asynchronous resolver minutes later |

⚠️ **`approval_from_the_ask` DELIBERATELY DOES NOT RE-CHECK CREDENTIALS, and that is not
an omission to close with a defensive re-check.** The asynchronous caller cannot supply
the same inputs — the account identity came off a signature-validated token inside the
request and is deliberately **not persisted onto the ticket**. A re-check fed weaker
inputs would refuse callers the real check passed: *a defect wearing a belt*.

⇒ Same reason `PromotionApproval.reason_with_suffix` moved onto the dataclass. Composed at
each door, the two would agree until somebody changed a separator — and an asynchronous
promotion would then be **distinguishable from a synchronous one on the row**, for no
reason a reader could guess.

### 11.3 🔴 THE NARROWING: BOUNCE-RECOVERY IS **NOT** BUILT, AND §6.3 SHOULD BE READ WITH THIS

§6.3 recovers a bounce-orphaned answer by reading the notification's `responded_at` off
the ticket's `notification_id`. **It is not built.** Measured: `NotificationResponse`
carries no such field — `notify_user_sync` captures the id off the opening ack frame into
a local capture dict and never surfaces it, across **fifteen** construction sites. So
`ticket.notification_id` stays NULL in this landing.

**What that costs, precisely:** after a bounce a pending ticket goes `stalled` and fires
the urgent notify, so **a human is told and nothing is lost silently.** What is missing is
the AUTOMATIC recovery of a keypress Rick made in the seconds before the bounce. The loud
path — the half the conditional ruling is actually about — is intact.

⚠️ **Rio ⚡, 2026-09-06, independently and as a second opinion**: the id *is* captured
internally and surfacing it is *"low-risk plumbing"*. That is HIS estimate, recorded as
his; it is the argument for the follow-up's priority and it should carry his name. It was
not the basis for scoping the work out — a shared model on a path this row does not own,
on the evening the 202 lands, was.

### 11.4 The re-validation, which is the guarantee the 202 gives up and has to buy back

Today's door holds the row lock across the ask, so the row cannot move between the
decision and the apply. `resolve_ticket` gives that up in three phases — **phase 2 holds
no connection, no lock, no transaction** — and buys it back in phase 3 by re-validating
under a fresh lock.

🔴 **A ticket whose transition is no longer legal resolves `superseded`, NOT `refused`,
and the distinction is the whole value of the branch.** A reader of a refused ticket
learns Rick said no. A reader of a superseded one learns the answer was fine and **the
world moved** — the only reading that sends them to look at what else touched the row.

### 11.5 Pocholo 📣's point, and the correction it produced in the code

His finding: §6.3's startup reconciliation *does not visibly inherit* §6.2's idempotency —
two writers, two contracts. **He was right about the doc and about the visibility, and the
code already held one contract via a shared helper.** Two things changed because of him:

1. `_mark_stalled` now says in its own docstring that it is the ONE writer of `stalled`,
   so a reader does not have to reconstruct that from the call graph.
2. **The deadline re-check moved out of the SQL filter and into Python, under the lock**
   (`require_overdue`). The filter chooses candidates; the check decides. A safety
   property that lives only in a query is invisible to every reader of the function and
   untestable without a database — and a third caller added later would inherit the state
   check and silently **not** inherit the deadline check.

⚠️ **One distinction worth keeping, since it now binds another sweeper**: §6.2's
*re-validate* and §6.3's *idempotency* are two different contracts. Reconciliation never
applies anything, so it has nothing to re-validate; it does inherit idempotency. Adopting
"§6.2" wholesale onto a sweeper may import a requirement that sweeper cannot express.

### 11.6 The receipts

**Green baseline recorded as a failing SET before any arm: EMPTY.**

| run | result |
|---|---|
| promotion surface, 10 files | **186 passed, 0 failed** |
| every unit file touching the tasks router, 39 files | **937 passed, 13 skipped, 0 failed** |
| the new guard file | **32 passed** |

**Five mutation arms, each killed by its OWN named test with the others green** — a
blanket redden proves only that a file can fail. Restores verified byte-exact by sha;
caches purged with `src/scripts/purge-pycache.sh` between arms, never a raw `rm -rf`.

| arm | newly red |
|---|---|
| the handoff (`background_tasks.add_task`) deleted | `test_the_handler_actually_hands_the_ask_off` |
| re-validation removed | `test_a_row_that_moved_under_the_ask_is_SUPERSEDED_not_refused` |
| the connection held across the ask | `test_the_ask_runs_with_NO_database_connection_open` |
| the deadline check dropped | `test_a_ticket_INSIDE_its_deadline_is_untouched_and_SILENT` |
| the precheck's refusal swallowed | `test_a_refused_caller_gets_403_and_NO_orphan_ticket_is_minted` |

### 11.7 🔴 A DEFAULT ARGUMENT BINDS AT IMPORT, AND IT GAVE ME FOUR WRONG ANSWERS

The first cut of the guard file monkeypatched `gate.get_asynchronous_enabled` and
`gate.is_manager_figure`. **Neither is ever looked up again**: both are captured as default
arguments when the module is read. The endpoint quietly took the SYNCHRONOUS path while
the test believed it had opened both gates, and reported **200** — which reads as *the fork
is broken*, not as *the test never reached it*.

⇒ **Patch what is resolved at CALL time, which is the module global the function BODY
reads.** `_ini_value` is looked up inside `get_asynchronous_enabled` on every call, so
faking it leaves both real gates running. Stubbing `promotion_is_asynchronous` itself
would have been easier and would have measured the stub.

### 11.8 Coverage, and a phantom branch measured rather than waved at

`task_promotion_resolver.py`: **130 statements, 0 missed.** Six branches report partial,
all of the form `NNN->exit`, all on a one-line `if …: return` **inside a `with` block**.

**They are phantom arcs — predicted by static analysis, never produced at runtime** — and
that is measured, not assumed. An 11-line probe with one variable:

| function | both branches exercised? | reported |
|---|---|---|
| `if/return` **inside** a `with` | yes | `9->exit` **partial** |
| `if/return` **outside** a `with` | yes | fully covered |

The second row is the positive control: without it the first would only show that the
instrument can report a partial. ⚠️ **Nothing was restructured to chase this**, and no
`pragma` was added: the repo's gate is a global `fail_under = 97` and the artifact affects
every `with`-block return in the tree equally. Python 3.13.7, this worktree, 2026-09-06.

---

## 12 · Stage 4: the gate that was open and had nobody walking through it

**Written after the build by Krishna 🦚, row `8ed76594`, 2026-09-09. Green-lit by Mr. Radio 🦉
with three named conditions, all three met and named below.** Every figure here was measured
in this worktree at `b26c32ee`; re-derive rather than quote.

### 12.1 The finding, and why it is a whole stage rather than a config line

Both gates shipped. Both were verified **open** — `task approval promotion ask asynchronous =
True` landed at `lupin-app.ini:1139` in `b2860542`, and `get_asynchronous_enabled()` returns
`True`. And for a day the feature **did not exist for anyone**, because the second gate is
per-call and no caller set it: the single `asynchronous=True` anywhere in the production tree
was a docstring.

⇒ **A capability every caller must remember to ask for is a capability nobody uses.** §5's
opt-in design is right about safety and was, on its own, a feature switched off by ergonomics.

⚠️ **§5.5's "no caller opts in" is now stale as written.** The accurate claim is *no
PRODUCTION caller*: `asynchronous=True` also matches ten sites across the two test files
that pin the path. Positive control for the search: 190 hits for the bare term across 77
files, so the instrument reaches.

### 12.2 The fix, and the seam it deliberately does NOT use

The MCP verb `task_transition` in `cosa_voice_mcp.py` now defaults `asynchronous` to `True`.
Three states, on purpose: `True` opts in · `False` demands today's synchronous path · `None`
omits the field.

🔴 **THE SEAM IS THE VERB AND NOT `task_transition_impl`'S OWN DEFAULT, AND THAT IS THE WHOLE
OF CONDITION 1.** Mr. Radio's condition was *"name that one caller, and nothing else"*.
`session_spawner.py` calls that impl too — a `->done` close during session spawn — so a
default one layer down would opt **it** in as well, putting a 25s poll budget inside a spawn.
The impl keeps its omit-unless-set contract, so every other caller of it still sends a
byte-identical request. Two guards pin this, because a default and an explicit argument are
different failures: one reads the impl's default, one reads the spawner's actual keywords.

### 12.3 Why opting in by default is safe on every OTHER transition — measured, not argued

The verb now sends `asynchronous: true` on **every** transition, not only promotions. The
async fork sits inside the promotion branch, under `get_enforcement_active()` **and**
`item.status == NOT_APPROVED` **and** `to_status != NOT_APPROVED` (§2's condition 2). Off that
path the field is one JSON key the handler never consults.

`test_the_opt_in_on_a_NON_promotion_row_is_INERT` drives the assembled app with both gates
open, a real boolean, and an already-`queued` row: **not 202, no ticket minted, no ask handed
off.** Its positive control is the byte-identical request against a `not_approved` row, which
does get the 202. Mutation-proven: flip the arm's row back to `not_approved` and it fails;
restore and it passes.

### 12.4 Condition 3 — the unreachability guard, and why it is not the arm the row asked for

The row asks for a test that a 202 leaves no optimistic "approved" row behind. **That defect
is UNREACHABLE today rather than merely unguarded**, so writing that arm would mean building
the browser opt-in *in order to* guard it. Mr. Radio's condition instead: pin the
unreachability.

🔴 **THE GUARD ASKS `tsc`, IT DOES NOT RESTATE THE TYPE.** A probe writes a `.ts` file that
hands `{ asynchronous: true }` to the real `TaskListStore.transitionTask`, under a tsconfig
that `extends` the repo's own — so the arm cannot drift from the settings the typecheck gate
enforces. Result: `error TS2322: Type 'boolean' is not assignable to type 'string'`. ~0.6s.

⚠️ **ITS CONTROL IS THE HALF THAT MATTERS, AND IT IS §5.5.2 RESTATED AS A TEST.** The
*string* `"true"` **compiles clean**. So the compiler refuses one shape only, the server's
`StrictBool` is the real defence, and an unreachability guard whose control did not compile
would be measuring a broken probe rather than a narrow type.

**And a census, because a probe interrogates one surface.** Five files carry an annotated
`extras` parameter — `render/taskVerbs.ts` (the producer), both stores, both renderers —
seven annotations, every one `Record<string, string | null>`. A sixth file appearing on disk
reddens a named arm.

🔴 **THE CENSUS ARM CAUGHT ITS OWN AUTHOR'S LIST.** It was written with **four** files in it;
the on-disk count named the fifth, `taskVerbs.ts`, which *builds* the map all four others
receive. That is the receipt that it discriminates rather than agreeing with whatever it is
handed — and the reason the hand-maintained list gets an on-disk counterpart at all.

### 12.4a 🔴 The census walked the wrong population, and §5.5.2 had already said so

**Found because Mr. Radio 🦉 ordered a client-side sweep before this row closed.** The census
above walks `multiplexer/**/*.ts`. **That is my corpus, not the population.**

⇒ **There are THREE client transition call sites, not two.** The third is
`notifications.js:10807` — `_transitionTask( taskId, toStatus, extras = {} )`, vanilla JS with
**no type system**, spreading `...extras` into the body and returning `{ ok: true }` for any
2xx. §5.5.2 already recorded that `notifications.js` has no types and that this was the reason
the guarantee had to move to the server. **My guard's own docstring then claimed every client
surface was typed** — a false *reassurance*, which is the worse half: a wrong instruction is
caught the first time someone follows it, a wrong reassurance disarms the reader who would
have caught it.

**So the two layers earn their safety differently, and the guard now says which:**

| layer | property | instrument |
|---|---|---|
| multiplexer `.ts` | unreachable **by construction** | `tsc` refuses the boolean |
| `notifications.js` | unreachable **by absence** — weaker | an absence arm, labelled as one |

⚠️ **AND ITS DEFECT SHAPE IS MILDER, measured rather than assumed.** Neither untyped call site
writes an optimistic row: `:13345` re-reads the pane (`if ( result.ok ) await
this.refreshTaskList()`), `:13805` advances a batch counter. A 202 there would over-report
success and then repaint the row's **real** state — bad, and **not** the "approved and stays
approved" false fact `TaskListStore.ts:229` produces. §5.5's table should not be read as one
defect across all three sites.

**The population, and its control.** `asynchronous` in `notifications.js`: **2 hits, both prose
in comments** (`:3559`, `:16545`). A hit is not a use, so the arm subtracts comment lines rather
than asserting on the substring. **Positive control: `park_reason`, 8 hits in the same file** —
it does carry transition fields, so the zero is a real absence and not a bad path.

### 12.5 Receipts

| run | result |
|---|---|
| the three prior async files, at `b26c32ee`, before any edit | **85 passed** |
| the new guard file | **13 passed** (12 before the absence arm below) |
| M1 — MCP default reverted `True`→`None` | **1 failed** (`test_the_MCP_verb_DEFAULTS_the_opt_in_ON`), 11 passed. Restore: 12 passed |
| M2 — client extras widened to admit a boolean | **2 failed** (the tsc probe, and the census on that one file), 10 passed. Restore: 12 passed, `git diff` on the `.ts` empty |

⚠️ **M1 KILLED EXACTLY ONE ARM AND M2 EXACTLY TWO, AND THAT CEILING IS PART OF THE READING.**
A break aimed at one line that reddens half a file is a syntax error until proven otherwise;
both runs held their total at 12.

### 12.5a 🔴 A COLLISION I COULD NOT SEE FROM MY OWN BRANCH — and the correction it forces

**Mr. Radio 🦉 found it, 2026-09-09 19:31 EDT.** Commit **`b76fe07b`** (2026-09-08 23:02, branch
`john-202-is-not-a-success`, **unmerged**) is **not an ancestor of `b26c32ee`**, so nothing I
measured client-side could see it. It is on **this same row**, and it already:

- teaches `notifications.js` to discriminate `response.status === 202` and answer
  `{ ok: false, pending: true, ticketId }` — so my §12.4a "reads any 2xx as success" is
  **true of my checkout and expires when that branch lands**;
- teaches `HoldingAreaStore` the same rule via the server's **body marker**, because
  `ApiClient.request` returns the parsed body and never surfaces the status;
- **already contains the optimistic-"approved" arm** —
  `task_list_store_202_leaves_no_optimistic_approval.test.ts`, three red arms with three
  controls still passing, **held on Rick's ruling rather than landed**;
- adds **92 lines to `test_the_asynchronous_promotion_resolves_or_shouts.py`**, the same file
  §12.3's arm appends to. **That is a textual conflict, not a semantic one**, and it is the
  integration decision this doc does not get to make.

⚠️ **THE HOLD ON THAT ARM SURVIVES THIS STAGE, which is the obvious worry.** It is held because
no production caller sends the opt-in. **The caller stage 4 adds is the MCP verb, not a
browser** — `transitionExtras` still constructs exactly three keys and cannot carry a fourth —
so the browser door stays shut and that guard's own control (the posted body carries no
`asynchronous` key) stays green.

🔴 **THE LESSON IS THE ONE THE WORKING RULES ALREADY STATE, and I still walked into it.** *"A
red count or a coverage list about 'the tree' must be run at the main line. Your own branch is
blind to exactly the defects its unmerged work repairs."* It is blind to **peers' repairs** the
same way, and a census that says "three call sites read 2xx as success" is a statement about a
sha, not about the repo. **Every client-side figure in §12.4 and §12.4a is bound to
`b26c32ee`** and must be re-derived against whatever line carries `b76fe07b`.

### 12.6 What this does NOT fix, said out loud

- **The optimistic-"approved" arm is WRITTEN, not owed** — `b76fe07b`, held on Rick's ruling.
  An earlier draft of this section listed it as owed, which was wrong: it was owed in my
  checkout and written in the repo. The guard in §12.4 is what makes the day of a widened type
  loud; it was never a substitute for an arm that already exists.
- **Nobody has measured how long Rick takes to answer.** Struck as unmeasured on row
  `3493ae9b`, still unmeasured here. If he is slower than the 25s budget then
  `awaiting_human_approval` is what callers normally see — a different promise from "the
  common case is invisible", and §5 should not be read as claiming the latter.
- **Rick's propagation ruling** (the verdict written to the row rather than returned in the
  response) would remove the false-fact defect at the root rather than guarding it. His own
  amendment calls that an argument nobody has driven. Nothing here builds on it.
