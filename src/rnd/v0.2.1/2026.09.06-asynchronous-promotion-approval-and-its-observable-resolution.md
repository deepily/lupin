...
| piece | what it is |
|---|---|
| `promotion_tickets` table | `id`, `task_id`, `to_status`, `requested_by`, `requested_at`, `resolves_by`, `state`, `approval_source`, `refusal`, `notification_id`, `resolved_at`, and the caller's original payload |
...
|---|---|
| resolved in time | `{ item, event }` — **byte-identical to today** |
...
## 6 · The condition, answered — three failure modes, and what makes each loud
