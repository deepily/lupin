...
**Agentic-pool occupancy (config dependency)**: a `HeartbeatPokerJob` is an `AgenticJobBase`, so it holds ONE agentic-pool slot for its entire lifetime (up to `max_duration_seconds`, default 12h) — a held slot, not a transient one. This is an approved design assumption with a deployment consequence: when N pokers run concurrently (§4 contemplates 2), the production agentic pool must be sized **≥ N+1** so the fast-lane and other agentic work are not starved. The prod default is `cj flow max concurrent agentic jobs = 1`, which a single long-lived poker fully saturates; **pool = 3 is the safer floor** (pool = 2 with 2 concurrent pokers = full saturation, zero headroom). Treat agentic-pool sizing as a deployment prerequisite of running pokers — recorded here as a config dependency of the design, not as a poker-class concern.
...
| **Dead-man's-switch** | 3 silent pokes in a row (no recipient response) | Fire `notify()` to user **once per silent streak** (a `dms_fired_this_streak` flag prevents re-firing every tick past threshold); **KEEP poking** (don't auto-die — user decides); **reset the silent-streak counter and the fired-flag when the recipient revives**, so a later streak re-escalates cleanly |
...
**Clean-exit guard** (prototype-learned): only termination signals posted *after* the poker's start timestamp count. The prototype `cascade_heartbeat_scheduler.py` captures `initial_size` at job start and matches only post-start content — without this guard, a stale termination signal from a *prior* run still sitting in `termination_topic` triggers a false clean-exit at tick 1.
...
| D1+D4 | Class spec + integration patterns | `HeartbeatPokerJob` class shape (fields, methods, lifecycle) bound to integration constraints: `QueueableJob` protocol satisfaction, `AgenticJobBase` parent contract, commons-store interaction patterns |
...
| I6 | Test pyramid | Unit (class + protocol-satisfy) + smoke (CJ Flow ingestion + `scheduled_at`) + integration (poker + recipient end-to-end on `:8000`, scheduled via `/api/test-suite/submit`) + E2E (real cascade replay on `:8000`, scheduled). 100% coverage per Lupin-wide mandate |
