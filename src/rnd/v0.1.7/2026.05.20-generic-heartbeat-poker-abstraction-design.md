...
|---|---|
| **Date** | 2026-05-20 |
| **Status** | 📝 **WIP — design conversation in progress** (parked end-of-day 2026-05-20) |
...
### Q3 — Relationship to existing daemon

**Options**:
- (a) **Replace** `cascade_heartbeat_scheduler.py` shell-script with the new Lupin-side agentic-job; migrate cascade case to it; shell script retires
- (b) **Co-exist** — Lupin-side abstraction validates on implementer-keep-alive use case first; shell-script daemon stays as cascade-only legacy; swap cascade-heartbeat after validation

**Tradeoffs**:
- (a) Cleaner long-term; one mechanism for both use cases; but bigger migration risk on a working system
- (b) Lower-risk; validates new mechanism on a new use case (implementer-keep-alive) before swapping a working one (cascade); but maintains two mechanisms during the bridge period

**Tiberius's lean** (not ratified): (b) — validate on the new use case first, then swap. Implementer-keep-alive doesn't exist yet, so any breakage we hit is on novel territory rather than regressing a proven path.

### Open Q — Where does the Watcher-mode Probe Protocol live?

The Observer-mode protocol already lives in `planning-is-prompting/workflow/plan-review-cascaded-common.md` (which is workflow-doctrine for cascaded review). The Watcher-mode protocol is structurally parallel BUT serves a different use case (implementer-watch, which is downstream-of-cascade, not part-of-cascade).

**Candidate homes**:
- (α) Same file — `plan-review-cascaded-common.md` as a sibling section to Observer-mode. Pro: consistency. Con: bloats a workflow-doctrine doc with something that isn't really cascade-doctrine.
- (β) New PIP file — `planning-is-prompting/workflow/implementer-watch-protocol.md` (or similar). Pro: clean separation. Con: more docs.
- (γ) Lupin-side — `lupin/src/docs/agents/implementer-watch-protocol.md` or similar. Pro: lives near the code. Con: doctrine spread across repos.

**Tiberius's lean** (not ratified): (β) — the implementer-watch use case is workflow-doctrine-shaped (governs cross-session behavior in a multi-persona setup), so PIP is the right repo. New file because the use case is structurally distinct from cascade-review.

### Open Q — Persona-allocation strategy under concurrent work streams
...
**Possible answers**:
- Pre-allocate roles at scenario-launch (manual)
- Pool-expansion (grow the persona pool — already grown from 6→8 in v0.1.7)
- Same-session-two-roles pattern (no extra persona consumed; recipient context-switches per tick)

Likely a runtime/operational concern handled by user + Manager judgment per scenario; the poker abstraction doesn't bake in any assumption.
