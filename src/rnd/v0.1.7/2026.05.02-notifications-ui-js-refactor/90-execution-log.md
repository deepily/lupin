...
| 3 | `ActionRequiredRenderer.ts` | ✅ DONE (gated on session-end commit) | 2026-05-11 late PM | 2026-05-11 late PM | (uncommitted — session-end batches with Phase 4) | NEW: `ActionRequiredRenderer.ts` (~360 LOC). 5 widget builders (interactive/submitting/responded/expired/cancelled); ownership claim via `dataset.phase6bOwner="true"` BEFORE any DOM write (Pass 2 A3); click → `respondAndAwait` (Pass 2 A1); tick → `.textContent` only (Pass 2 a2 NO-RAF; spy-asserted); inline error stripe on `failed` state. 34 tests (≥21 AC5 floor). c8 100% with `cssEscape` polyfill `c8 ignore start/stop` (mirrors NLR.ts:423 precedent). |
| 4 | `TtsChromeRenderer.ts` | ✅ DONE (gated on session-end commit) | 2026-05-11 late PM | 2026-05-11 late PM | (uncommitted — batched with Phase 3) | NEW: `TtsChromeRenderer.ts` (~155 LOC). Dual-subscription (state_change + chunk_decoded) → shared `pendingRender` flag → single RAF (Q-B9 + F-13). Test-injectable RAF lets storm-safety tests deterministically flush. Click handlers wire to AudioStore.pause/resume/stop/skip. `replaceChildren` for atomic full-pane swap (no phantom-branch issues). currentTrackName omitted (Phase 0 prereq #3 pending). 20 tests (≥13 AC5b floor). c8 100% on first try. |
| 5A | `JobStore.delete()` + AC2d tests (DOD 5A-1..5A-8) | ✅ DONE (gated on commit auth) | 2026-05-12 AM | 2026-05-12 AM | (uncommitted — awaiting user) | All 8 DOD rows green. `delete(idHash)` added to `JobStore` interface + impl (captures bucketName + index + job, splices out, deletes from indexById, emits `removed`; returns closure that splice-inserts at original index, restores indexById, emits `added` with `to: bucketName`). Nonexistent idHash → no-op closure + zero events emitted (5A-6). New test file `src/tests/unit/multiplexer/jobstore_delete_api.test.ts` (flat per existing convention; design plan's `stores/` subdir path was inaccurate). 11 new tests via `node:test`. JobStore.ts test cohort: **29/29 PASS** (18 existing + 11 new). c8: **100% stmts/branches/funcs/lines on JobStore.ts**. tsc + eslint clean. One `c8 ignore next` added for `idx < 0` invariant guard mirroring the existing `onStateTransition`/`onRemoved` pattern at lines 247 + 282. |
| 5B | Delete-button click handler on `JobsPaneRenderer` (DOD 5B-1..5B-11) | ✅ DONE (gated on commit auth) | 2026-05-12 AM | 2026-05-12 AM | (uncommitted — batched with 5A) | All 11 DOD rows green. NEW `JobsPaneApiClient extends JobHistoryApiClient` interface in `JobsPaneRenderer.ts` adds `delete<T>(path)` (production passes singleton `ApiClient` which conforms structurally). Click-delegation now dispatches `.job-delete-button` clicks to `handleDeleteClick()` BEFORE the card-header toggle path (preserves Pass 2 F23 invariant under active-button semantics). Q-B10 flow: idempotency via `Set<string> deleteInFlight`; `stores.jobs.delete(idHash)` for optimistic local removal; `api.delete(/api/queue/${queueName}/${idHash})` where queueName uses `UI_STATUS_TO_SERVER_QUEUE` (note `running → run` legacy mismatch). 2xx + 404 → discard restoreState; 5xx + non-ApiError Error → `restoreState()` + inline `.job-card-error-stripe` (role=alert, aria-live=polite) on restored card. `stripInertnessMarkers()` called at end of `renderAll()` removes `aria-disabled` + `tabindex` + `title` from every `.job-delete-button` so AC2a/AC2b grep guards return zero hits post-mount. 9 new tests (Tests 21–29); existing 23 Phase 6a tests preserved. `jobs_pane_renderer.test.ts`: **32/32 PASS**. Full multiplexer sweep: **602/602 PASS** (was 593 post-5A, +9 new). c8: **100% stmts/branches/funcs/lines on JobsPaneRenderer.ts**. tsc + eslint clean. Implementation deviation noted: design plan's "4 markers" list (`data-phase6-pending`, `aria-disabled`, `cursor: not-allowed`, `title`) is partly aspirational — `data-phase6-pending` is on `#tts-pane` not `.job-delete-button`, and `cursor: not-allowed` is a CSS rule keyed on `[aria-disabled]` so stripping the attribute lifts the cursor. Actual strip is 3 attributes: `aria-disabled`, `tabindex`, `title`. `SimpleApiStub` widened to satisfy `JobsPaneApiClient`; 4 inline narrow `api: JobHistoryApiClient = {…}` stubs at lines 223/464/509/555 left as-is since their tests don't exercise delete. Test 18 (formerly "clicking the disabled button does NOT toggle") still passes with renamed semantics: clicking now dispatches delete but the card-header toggle remains gated, which is what the test still asserts via the detached-node classList check. |
| 6 | CSS port + page shell + boot wiring | ✅ DONE (gated on commit auth) | 2026-05-12 PM | 2026-05-12 PM | (uncommitted — awaiting user) | All Pass 2 A7/A8 ordering invariants enforced. NEW `action-required.css` (295 LOC, ≤500 ✓) + NEW `tts-chrome.css` (187 LOC, ≤700 ✓), both stylelint clean (auto-fix normalized `#ffffff → #fff`, `rgba(...) → rgb(...)`, `word-wrap → overflow-wrap`). `.stylelintrc.json` extended with `selector-disallowed-list` overrides for both files (layer-2 F28 fail-build rule). `multiplexer.html` gains both `<link rel="stylesheet">` entries. `shared/types.ts` `BootCompletePayload.handlers` extended with optional `actionRequiredRenderer?: string` + `ttsChromeRenderer?: string` (F11 runtime-unconditional, F12 type-optional). `render/index.ts` barrel exports `createActionRequiredRenderer` + `createTtsChromeRenderer` + their types. `boot.ts` wires both new renderers in canonical AC9 order (notifications → jobs → actionRequired → ttsChrome); inline comment marks the A8 ordering invariant; `transports.audio.start(...)` lands after every renderer mount (AC9b precondition). `#tts-pane` `hidden` + `data-phase6-pending` markers lifted in `boot.ts` post-`ttsChromeRenderer.mount()`. `dev-tools.html` line 145 description updated to reflect Phase 6b live status. `boot.js` rebuilt to `boot.666863463c09.js`, **gz = 34,647 bytes** (AC7 ceiling = B6a 31,484 + 8,192 = 39,676; ✅ +3,163 vs B6a). Multiplexer page serves 200 on `:7999` with both CSS links + tts-pane scaffolding present. Full unit sweep: **602/602 PASS** (no regressions). tsc + eslint clean. |
| 7 | Smoke + cross-phase verification (`:7999` AI-discretionary) | ✅ DONE (gated on commit auth) | 2026-05-12 PM | 2026-05-12 PM | (uncommitted — awaiting user) | NEW `src/tests/smoke/test_multiplexer_phase6b_smoke.py` — 6 Playwright sub-tests covering AC2a/AC2b/AC8a/AC8b/AC9/AC9b/AC10d. **All 6 PASS in 6.76s on :7999.** AC10e cross-phase cascade: Phase 5 `test_phase5_functional_smoke` + Phase 6a `test_phase6a_data_phase6_pending_exact_count` floor updated from ≥3 / ==1 to **==0** (Phase 6b lifts the final placeholder on `#tts-pane`); Phase 5 `test_phase5_boot_complete_handler_handshake` substring filter tightened from loose `"notificationsRenderer" and "mounted"` to literal `"notificationsRenderer:mounted"` (no quotes between key/value) so the JSON-form boot_complete line isn't double-counted. Full cross-phase sweep: tsc clean + eslint clean + stylelint clean + 602/602 unit + 14/14 Phase 5+6a+6b smoke + AC2e grep ban green (no `.innerHTML =` / `rawHTML(` / `.outerHTML =` in `actionRequiredInteractive.ts` / `ttsChrome.ts`). c8 100% confirmed on all 9 Phase 6b-touched TS files (3 stores + 4 renderers + 2 templates). |
...
| AC2a | `data-phase6-pending` 0 hits on action-required widgets post-mount | ✅ (post-Phase 7) `test_phase6b_no_pending_markers_after_mount` PASS; 0 hits page-wide |
| AC2b | `aria-disabled="true"` 0 hits on action-required widgets post-mount | ✅ (post-Phase 7) `test_phase6b_no_pending_markers_after_mount` PASS; 0 hits inside widgets |
| AC2c | 1 MutationObserver childList entry per widget; 4 markers gone | ✅ (post-Phase 3 ActionRequiredRenderer unit) `action_required_renderer.test.ts` covers MutationObserver count + inertness-strip atomicity |
| AC2d | `vitest jobstore_delete_api.test.ts` green | ✅ (post-Phase 5A) 11/11 PASS via `npx tsx --test` — see note below re: vitest vs node:test |
...
| AC5 | `action_required_renderer.test.ts` ≥21 PASS | ✅ 34/34 PASS (post-Phase 3) |
| AC5b | `tts_chrome_renderer.test.ts` ≥13 PASS | ✅ 20/20 PASS (post-Phase 4) |
| AC5c | `jobs_pane_renderer.test.ts` AC5c cases (≥6 new) | ✅ (post-Phase 5B) 9/9 new AC5c cases PASS; cumulative 32/32 |
| AC6 | `c8 --100` on all new + edited multiplexer TS files | ✅ (post-Phase 7) **100/100/100/100 confirmed across all 9 touched files** in a single c8 sweep: 3 stores (ActionRequiredStore, AudioStore, JobStore), 4 renderers (ActionRequiredRenderer, JobsPaneRenderer, NotificationsListRenderer, TtsChromeRenderer), 2 templates (actionRequiredInteractive, ttsChrome). |
| AC7 | `boot.js` gz ≤ `B6a + 8192` = **39676 bytes** (B6a = 31484, captured 2026-05-11 PM) | ✅ (post-Phase 6) **34647 bytes** = B6a + 3163; +5029 bytes of headroom against ceiling |
| AC8a | smoke functional + no-pending-markers | ✅ (post-Phase 7) `test_phase6b_functional_smoke` PASS — 3 prompts × 3 response_types render as interactive widgets; tts-pane chrome renders; tts-pane hidden + data-phase6-pending lifted |
| AC8b | smoke perf gate (50 prompts, first paint <200ms) | ✅ (post-Phase 7) `test_phase6b_perf_gate` PASS |
| AC9 | smoke `boot_complete_handshake` (4 stable lines, canonical order asserted) | ✅ (post-Phase 7) `test_phase6b_boot_complete_handshake` PASS — `notifications → jobs → actionRequired → ttsChrome` canonical order asserted; JSON-form line lands after the 4 stable lines |
| **AC9b** (NEW per Pass 2 A8) | smoke `audio_chunks_arrive_after_mount` (4 `:mounted` lines BEFORE first `store_audio_chunk_decoded`) | ✅ (post-Phase 7) `test_phase6b_audio_chunks_arrive_after_mount` PASS via synthesized chunk event + console-marker probe |
| AC10 | cross-phase verification sweep | ✅ (post-Phase 7) tsc + eslint + stylelint + 602/602 unit + 14/14 Phase 5+6a+6b smoke + AC2e grep ban + c8 100% all green |
| AC10b | action-required.css ≤500, tts-chrome.css ≤700 + stylelint exit 0 | ✅ (post-Phase 6) action-required.css = 295 LOC, tts-chrome.css = 187 LOC, stylelint `*.css` exits 0 |
| AC10d | 3-layer CSS scope-leak detection (grep + stylelint + canary) | ✅ (post-Phase 7) Layer-1 grep clean (no `*`/`html`/`body`/`:root` in the new CSS); Layer-2 stylelint clean; Layer-3 canary `test_phase6b_css_canary_no_body_drift` PASS — disabling both Phase 6b stylesheets produces zero body-style drift on color/backgroundColor/fontFamily/margin/padding |
| AC10e | Phase 5 + 6a `pending_count` regression with floor=0 | ✅ (post-Phase 7) `test_phase5_functional_smoke` + `test_phase6a_data_phase6_pending_exact_count` updated to assert **==0** (Phase 5 was ≥3, Phase 6a was ==1, Phase 6b lifts the last placeholder on `#tts-pane`); both PASS |
