...
| AC6 | `c8 --100` on all new + edited multiplexer TS files | 🟡 partial: ✅ Phase 1 (ActionRequiredStore + AudioStore + NotificationsListRenderer = 100%); ✅ Phase 2 (actionRequiredInteractive + ttsChrome templates = 100%); ✅ Phase 3 (ActionRequiredRenderer = 100% with cssEscape `c8 ignore start/stop`); ✅ Phase 4 (TtsChromeRenderer = 100% on first try); ✅ Phase 5A (JobStore = 100%); ✅ Phase 5B (JobsPaneRenderer = 100% stmts/branches/funcs/lines on first c8 pass after removing dead idempotency branch); Phase 6 pending |
