...
- **Optimization Plan**: [2026.02.05-peft-trainer-optimization-plan.md](2026.02.05-peft-trainer-optimization-plan.md) - **🔄 PHASE 1 IN PROGRESS** - Three-phase plan to improve PEFT training accuracy from 85% to 96%+. Analysis of 32 commands reveals 5 struggling commands (50-67% accuracy) due to semantic ambiguity, alias fragmentation, and implicit context. Phase 1: Quick wins (receptionist keywords, weather templates, 2x sample size). Phase 2: Disambiguation (contrastive training, structured negatives). Phase 3: Advanced optimizations (class-weighted loss, curriculum learning). Target: Ministral-8B-Instruct-2410 model.
