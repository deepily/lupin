...
- **Item D — legal-transition graph — ✅ BUILT, green+held** (2026-06-15). `LEGAL_TRANSITIONS` (derived from the enums: every non-terminal → every other status; terminals have no out-edges) + the no-op/terminal check in `validate_transition` now consults the explicit graph (D-DELTA-1: prepended, behavior-preserving; receipt/blocked/dropped payload rules untouched). Mirror-edge regression (D-DELTA-2) derives the live hook's edges PROGRAMMATICALLY from `task_store_mirror.STATUS_TRANSITIONS` (+ queued seed + backward in_progress/review→queued) and asserts each ∈ LEGAL, terminal-source 422s, no-op 422s. Tests: 210/210 pass; `task_store_rules.py` **100% (118 stmts / 72 br)**. **Reviewer: ✅ APPROVE-WITH-DELTAS (2026-06-15).** D-DELTA landed: a `from_status`-validity guard in `validate_transition` (free VARCHAR, no enum CHECK, now-LIVE write path) short-circuits an unknown source to a DATA error — never a KeyError on the `LEGAL_TRANSITIONS` lookup; symmetric with the `to_status` short-circuit + covering test. NOT committed (held in working tree atop `0ca22758`; awaiting Tiberius's fold authorization + the final D-delta/E review).
- **Item E — chase consumer + I4 — ✅ BUILT, green+held** (2026-06-15). New `src/cosa/rest/task_chase_consumer.py` — `TaskChaseConsumer` daemon (ghost-job-sweeper idiom: `Event.wait` loop, exception-guarded) that finds overdue blocked items, signals a chase nudge, re-arms `next_chase_ts` (backoff), stamps a `chased` audit event — **NEVER auto-transitions**. Repo support: `query_chase_due` + `apply_chase` (status untouched). INI keys (lockstep app.ini `[Lupin: Baseline]` + splainer): `task store chase enabled` (**default False** — daemon inert: `sweep_once` no-ops, `start()` refuses to spawn), `…interval seconds` (300), `…backoff seconds` (1800). Time/DB/repo/signal all injectable → 100% unit-testable. Tests: 254/254 (full task-store footprint); `task_chase_consumer.py` **100% (61/12)**, `task_repository.py` **100% (68/8)**. **I4 SCOPING:** implemented as the read-only `stall_report` (store-derived overdue-blocked surfacing); the hook-marker I4 (sessions-that-should-write-but-don't) stays in the hook lane (Phase-2 flag-once markers) — flagged as a documented follow-on for the reviewer to rule. Activation (wire `start()` into server boot + flip the flag) is the deliberate opt-in step, NOT done here. NOT committed (held atop `0ca22758`).

### Backlog complete (A–E)
All five buildable items green+held. **Reviews:** A ✅ APPROVE (cleared) · B ✅ APPROVE (all 6 criteria, no defects) · D ✅ APPROVE-WITH-DELTAS (delta landed) · C + E pending the one final from-brief reviewer (Tiberius spawns it to verify the D-delta + review E together, sidestepping the persona-null wake issue).

**Deferred to Phase 2.2:**
- **Item F** — T4 DM auto-create-on-review-request (unresolved acked-ledger-scope ruling).
- **Hook-marker I4** — server-side detection of sessions-that-should-write-but-don't, reading the hook lane's Phase-2 flag-once markers. Deferred (Tiberius ruling 2026-06-15): a server reading hook-lane flag-once FILES is fragile cross-lane coupling. Item-E's read-only `stall_report` (store-derived overdue-blocked surfacing) is the accepted Item-E I4; the hook-marker variant is the follow-on.

**Commit state:** A/C/B in held commit `0ca22758`; **D + E + the D-delta guard held uncommitted** atop it. Tiberius authorizes folding D+E+guard into a held commit once the final review clears; **push stays Rick's** gate.
