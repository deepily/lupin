...
- **INFERENCE (broad net):** idle ≈ alive (by **event-file `ts` PRIMARY**, `commons_who` secondary — N3) **AND** quiet (no heartbeat events + no commons activity) for ≥ `quiet_threshold`. Catches every quiet session, including those that never emit a beacon. Heuristic ("quiet" can mean a long tool-run).
  - **⚠️ THRESHOLD INVARIANT (F3 — Mr. Radio integration finding, 2026-06-04):** the inferred branch needs `quiet_threshold ≤ age ≤ alive_threshold` off the single `last_activity_ts`, so the window is non-empty ONLY when **`quiet_threshold < alive_threshold`**. The original defaults (`idle_threshold=900 > alive_threshold=600`) made the window EMPTY → the inference half was config-dead → the roster silently degraded to **declared-only**, defeating the hybrid. **Fix:** (a) rename `idle_threshold`→`quiet_threshold`; (b) default `quiet_threshold=300 < alive_threshold=600`; (c) **fail-fast config invariant** — `ArbiterConsumerJob` init raises if `quiet_threshold ≥ alive_threshold` (so the bug-class can't reship silently). Lock with a config-validation regression test + an INFER path test (do NOT hardcode `alive=True` — that hollowed the original unit test). **Bounded-window note:** a session quiet beyond `alive_threshold` with no beacon is treated as presumed-offline; the **declared beacon (sticky, alive-independent) backstops long-idle**, so inference is the backup for *missed* beacons within `[quiet_threshold, alive_threshold]`. **v2.1 robustness (flagged):** decouple `alive` (from `commons_who`) from `quiet` (from event-ts) to remove the single-timestamp window bound entirely.
