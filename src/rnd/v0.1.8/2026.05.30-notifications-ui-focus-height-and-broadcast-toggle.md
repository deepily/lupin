...
1. **Focus-mode height boost** — when CC session focus mode is ON, the focused sender card's per-day message lists grow 50% (250px → 375px) so more of the in-focus conversation is readable; non-focus mode unchanged.
...
| `TestFocusMode::test_focused_card_messages_get_height_boost` | `src/tests/e2e_ui/test_cc_session_strip_and_focus.py` | Focused card `.date-accordion-messages` = 375px in focus, 250px default/exit |
