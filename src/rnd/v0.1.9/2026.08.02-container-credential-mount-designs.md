...
### Landing log — 2026-08-02

| time | step | result |
|---|---|---|
| 18:03 | `systemctl --user stop lupin-credential-watcher.service` | `inactive` |
| 18:03 | fleet warning posted to `coordination` | sent — the sanctioned bounce script only does `restart`, so its ack-confirmed warning does **not** cover a force-recreate |
| 18:04 | `docker compose up -d --force-recreate` both | Recreated → Started; `:7999` boot #8 at 18:04:38 |
| 18:05 | mounts | both volumes + the `sessions/` bind present in both containers |
| 18:05 | **shadowing check, from inside** | `sessions/` → **5026 entries** in both. Not shadowed |
| 18:05 | health | `:7999` 200 · `:8000` 200 |
| 18:10 | Rick's `claude /login` on **dev** | credential present 18:10:31 |
| 18:12 | **live probe dev** | **`PONG`** |
| — | `claude /login` on **test** | ⏳ still owed |

Test-credential env (`LUPIN_TEST_INTERACTIVE_MOCK_JOBS_*`) was recovered from the running container before recreating — this session lacked those vars, and compose placeholders would have silently poisoned both new containers.

**Delivery note**: two `converse` calls 503'd immediately after the recreate. That is row `b93841ca` — a restart wipes `ws_manager`, so blocking asks report "user offline" while the user is present. Instructions went out as a `notify` instead of storming the ask channel; the ask succeeded once the client reconnected.

### Side finding — the CLI version skew is real, pre-existing, and NOT fixed by this

Rick noticed the container reporting `2.1.161` against the host's `2.1.220`. Measured:

| | CLI |
|---|---|
| host | 2.1.220 |
| `lupin-rest-dev` (post-login) | 2.1.220 — **self-updated during the login** |
| `lupin-rest-test` (pre-login) | 2.1.161 — the image's baked-in version |

The binary resolves to `/home/rruiz/.local/share/claude/versions/2.1.220` — **`.local`, not `.claude`**. That is *outside* the volume this change makes persistent, so the self-update lives in the container's throwaway layer: **every recreate reverts to 2.1.161 and self-updates again on first use.**

This predates the change and the change does not address it. Recorded rather than left to be read as covered by a green. Two options if version stability is wanted, both needing a separate ruling: bake a current CLI into the image, or persist `~/.local/share/claude` the way `~/.claude` now is.
