...
## Addendum (post-commit, 2026-07-15) — the `/clear`-invisibility nuance on candidate (b)

Surfaced by a **live data point** during a manager continuity handoff (Mr. Radio, thread `eb62aeab`):
his transition was a **`/clear` context-reset that KEPT session_id `da517b03`** (and the persona
name), **not** a re-spawn — so from a correspondent's vantage the id read as continuous the entire
time, correctly.

**The refinement this forces on candidate (b)**: a session-id suffix disambiguates distinct
**sessions**, but a `/clear` re-spin is **invisible at the session_id layer** — the same `da517b03`
can carry two different context-arcs across a `/clear`. Therefore **id-equality alone cannot
distinguish "one continuous session" from "same-id, post-`/clear` fresh context."**

**Why this does NOT weaken the recommendation** — it *bounds* it honestly:
- For the **operator-perception** goal (sightings #3/#4), this is the *desired* behavior: a `/clear`'d
  seat genuinely **is** the same session with fresh context; you *want* it to read as continuous, not
  as a new bearer. The suffix still correctly separates the cases the bug is about — two *distinct*
  sessions wearing the same freed name.
- For the **fleet-addressing** goal (#1/#2), routing to a session_id is *still* correct across a
  `/clear` (the id is stable), so the suffix-aware resolution extension is unaffected.
- The only residual: if a use case ever needs to distinguish *context-arcs within one session_id*
  (e.g. "which /clear generation authored this?"), the session_id suffix is insufficient and a
  **per-arc/boot token** would be required. That is **out of scope** for this bug (freed-*name*
  reuse across *sessions*), but noted so the implementer does not over-claim the suffix as an
  arc-level identity.

**Net**: candidate (b) remains the right structural fix; this addendum just records the honest edge
of what a session-id suffix proves (session identity, not context-arc identity). Carry into the impl
lane if `e2f11f6f` rules build.
