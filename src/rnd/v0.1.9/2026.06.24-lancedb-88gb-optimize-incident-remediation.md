...
| 2026-06-25 | `rebuild_lancedb_table.py` (mod) | **90.46 G → 1.07 G (EXECUTED)** | in-container staged rebuild, 176,877 rows preserved — see §8 |

---

## 8. Execution Log — Phase A REBUILD (2026-06-25, executed)

**Status**: ✅ **PHASE A COMPLETE & VERIFIED GREEN.** Plan `mcp-is-off-no-snappy-graham.md`.

**Deviations from the §3 runbook (both ratified by Rick mid-flight, 2026-06-25):**
1. **Swap mechanism**: the *canonical* `phase_swap` does **drop+create** (reclaims in place),
   NOT the rename→`__corrupt_bak` the §3 narrative describes (rename_table is
   `NotImplementedError`; the script author chose drop+create). So `backfill`/`reclaim` are
   no-ops and there is no parked `__corrupt_bak` to hand off. The 3-bucket handoff collapsed:
   the 90 G delete lives *inside* the swap (one blocking command, token-cheap to wait on).
2. **Rollback net**: added `--keep-rebuilt` (retain `__rebuilt` through the server bounce) +
   a `drop-rebuilt` phase to `rebuild_lancedb_table.py`. Net stack = DATA01 `__rebuilt`
   (inner, retained through P6) + a **clean** `__rebuilt` copy on DATA02 (outer — used in
   place of re-syncing the 90 G *broken* mirror; clean+current+1 min vs broken+30 min).

**Venue**: both rest containers were Exited, so every lance-API phase ran via a **transient
`docker compose run --rm --no-deps --entrypoint python lupin-rest-test`** one-off — same
image (lance 0.36.0 / lancedb 0.30.2, V2 core), inode-identical `./src` mount, NOT host venv.

**Sequence & results:**
| Phase | Result |
|---|---|
| P0 preflight | BROKEN confirmed: `list_versions()` hung >2 min; 38,704 manifests on disk span v~36,659→75,362, ancestors below ~36,659 missing → REBUILD (in-place `optimize()` non-executable) |
| P1 build | `__rebuilt` = 176,877 rows, clean chain (to_arrow 9.4 s, write 12.3 s) |
| P2 quiesce | both rest containers Exited; **cleared a runaway probe container** (timed-out `list_versions` left it Up, holding `_versions/` open) before proceeding |
| P3 outer net | clean `__rebuilt` (1.1 G, 3 files) → DATA02, du + file-count parity |
| P4 swap `--keep-rebuilt` | dropped broken 90.46 G in place, recreated fresh: 176,877 rows, **clean chain @ v1**; DATA01 16 G→106 G free |
| P5 canary | reopen read-only (servers down): `list_versions()` **0.00 s**, 1 version, sample read OK |
| P6 bounce | `docker compose up -d` dev+test → both **healthy ~30 s**, logs "Opened input_and_output_tbl w/ 176,877 rows" + "Application startup complete", no crash-loop (passlib/bcrypt `__about__` traceback is pre-existing/benign — `/auth/login` 200) |
| drop-rebuilt | DATA01 net retired after green; DATA02 clean copy retained as durable backup |

**Final**: `input_and_output_tbl` rows=176,877, version=1, **1.07 GB**; `__rebuilt`/`__corrupt_bak` absent. **DATA01 107 G free / 94 %** (was 16 G / 100 %). **~89 GB reclaimed.**

**Owed / next (Bucket 3, post-restart):**
- **Phase B** standing compaction (Decision 4 overturned by Rick 2026-06-25 — wants compaction stood up): preflight-guard module + lifespan ENQUEUER + INI keys + tests. *Premise correction: there is NO auto-compaction today — that absence is why it regrew; Phase B must be built for it to "run normally."*
- **Decision #6 amendments** to `lancedb_incident_recovery_2026_06_23.py` (header skew note + missing-manifest preflight) — still owed, non-destructive.
- **Uncommitted working-tree change**: `rebuild_lancedb_table.py` (`--keep-rebuilt` + `drop-rebuilt`). Commit at session-end; push stays Rick's word.
