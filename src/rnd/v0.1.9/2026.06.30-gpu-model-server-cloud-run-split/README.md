...
| [07-vpc-subnetwork-fail-loud-guard.md](./07-vpc-subnetwork-fail-loud-guard.md) | Follow-on `cb0e145a` (Cheech): FAIL-LOUD fix for the 06 §2 subnetwork trap — cross-variable `validation` on the module `vpc_subnetwork` rejecting network-set-with-subnetwork-empty at PLAN time (silent-skip rejected → would deploy no-egress INTERNAL_ONLY) + aligned the stale env-level description. RED/GREEN/compat proof via `terraform plan` (validate does NOT fire var validation); fmt+validate clean; no apply, tfvars untouched. Commit HELD. | *[referenced file removed in the 2026-09-22 R&D cleanup (Option A), ticket `3a2f726b`; recover with `git log --diff-filter=D -- <path>` then `git show <sha>^:<path>`]*
| [08-gcp-pgvector-propagation-and-terraform-reconcile.md](./08-gcp-pgvector-propagation-and-terraform-reconcile.md) | ✅ **BRING-UP COMPLETE (2026-07-07)** — GCP leg of v0.2.0 pgvector migration (task `c845346a`, Cheech): deploy `-r3` @ `4748ab2a` (self-consistent baked image, `./src` mount-shadow removed), schema → head `e1f2a3b4c5d6` via boot auto-migrate (`CREATE EXTENSION vector` + drop-HNSW), green-bar (health / backend=postgres / keystone exact-scan / no-HNSW index), VM **TERMINATED**. terraform reconcile `0 add/1 change/0 destroy` (benign null-drift, NO apply); sibling `c3fafac5` CLOSED. 2 deferrals → Rick (IAP :7999 firewall rule, authenticated calc E2E). |
