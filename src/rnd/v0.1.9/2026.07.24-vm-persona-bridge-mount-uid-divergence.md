...
## ✅ FINAL PLAN v3 — incorporating local-expert GO-WITH-CHANGES (OPERATIVE)

Second reviewer (local LLM expert, 2026-07-24) approved the v2 core design (container stays 1001; `group_add`; sessions bind over the nested path; setgid dir; `fchmod(fd,0o660)` before same-dir `os.replace`) with **three blockers**, all accepted. v3 = v2 with these amendments:

**Blocker 1 — `makedirs(mode=0o2770)` does NOT enforce 2770** (umask strips group-write; existing dirs unchanged; setgid-bit-at-create is not portable).
→ After `makedirs`, add an **explicit `os.chmod( session_dir, 0o2770 )`**. Add a unit test that runs under a **restrictive umask** (e.g. `0o077`) and asserts the dir ends up `0o2770` and files `0o660`.

**Blocker 2 — `${HOME}` short-bind is unsafe** (a root/automation HOME or missing source silently binds/creates the wrong empty dir).
→ Use **long bind syntax with `create_host_path: false`** and an **explicit required deploy var** (fail-loud if unset), and **parameterize the GID** (no hardcoded `1721846087`). In `cloud-gpu.env`:
```
LUPIN_HOST_SESSIONS_DIR=/home/admin_rickruiz_altostrat_com/.claude/sessions
LUPIN_BRIDGE_GID=1721846087
```
Compose (`rest` service):
```yaml
    group_add:
      - "${LUPIN_BRIDGE_GID:?set LUPIN_BRIDGE_GID}"
    volumes:
      # …existing…
      - type: bind
        source: ${LUPIN_HOST_SESSIONS_DIR:?set LUPIN_HOST_SESSIONS_DIR}
        target: /home/rruiz/.claude/sessions
        bind:
          create_host_path: false        # fail if the host dir is absent — never silently create an empty one
```

**Blocker 3 — verification must exercise the REAL cross-writer replace path**, not `cat`/generic temp files (that transition is exactly what broke prior designs):
1. host hook creates a bridge;
2. container runs `set_voice_persona()` (the real `atomic_write_json` replace) → assert **owner 1001, group = `LUPIN_BRIDGE_GID`, mode 0660** (numeric IDs);
3. host reads it, then rewrites it → re-assert numeric owner/group/mode;
4. container reads again — must succeed.

**Also folded (expert recommendations):**
- Backfill uses explicit **`chgrp $LUPIN_BRIDGE_GID` + `chmod 660`** on existing `cc-*.json`, and `chmod 2770` on the dir.
- Assert **numeric** `%u %g %a` after every write (names can mislead across the uid boundary).
- Verify **both** mounts (`claude-creds` volume AND the sessions bind) via `docker inspect` post-recreate.
- After `--no-deps --force-recreate lupin-rest`: confirm container **health**, **OAuth**, and **`notify()`** still work.
- **Runbook/security note**: the shared group is a **security boundary** — any member can *replace any bridge*. Acceptable here only because the group is effectively single-member per side; must be re-evaluated if the gid ever gains members.

**Status**: design approved by two independent reviewers with the above changes + a passing bidirectional runtime test. This is the plan to implement.
