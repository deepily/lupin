...
**Status**: IMPLEMENTED — Changes 1+2 committed on `worktree-cheech-orphan-bridge`, 100% unit
coverage on new code, Change 1 proven against real Postgres. :8000 integration test committed
(post-merge gate). Awaiting Mr. Radio's review + merge (held for Rick's awareness — fleet-wide
reap-semantics).
**Scope ruling**: **A+B** (Mr. Radio, 2026-07-15) — durable eviction folded in, not filed separately

> **Implementation note (2026-07-15, Cheech 🌿):** the durable mechanism landed as the
> **roster-query exclusion on the persisted `session_reaped` marker** (§4 Change 1) rather than a
> literal `is_hidden` write — Mr. Radio's blast-radius check proved `is_hidden` over-hides (every
> history/conversation query filters `is_hidden==False`; it is the user's clear-conversation
> soft-delete), so we scoped the eviction to roster-visibility only, exactly as he pre-authorized.
> Change 2 (the arbiter janitor sweep) ships behind a **default-OFF** flag
> (`arbiter orphan bridge sweep enabled`) so merge is byte-identical to today until Rick activates
> it. Commits: design `56ecebee` · Change 1 + unit `22529ac6` · docs `26ae789a` · reaper module
> `fba95a42` · arbiter poll-loop `46785ff4` · factory/flag `1814ade4` · integration `e82e9347`.
