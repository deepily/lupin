...
async def main():
    ...
    a_user = get_content_analysis_prompt(
        ...
        max_source_chars = 50000,   # 08-04 analysis literal
    )
    ...
    s_user = get_script_generation_prompt(
        ...
        audience                = cfg.audience,
        audience_context        = cfg.audience_context,
        max_source_chars        = 30000,   # 08-04 script literal
    )
    s_resp = await client.call_for_script( system_prompt=system, user_message=s_user )
    _report( "SCRIPT", s_resp )
    open("/tmp/claude-1001/-mnt-DATA01-include-www-deepily-ai-projects-lupin/effddeae-65bd-45d1-8b1b-50fd50c94adc/scratchpad/captured_script_body.json","w",encoding="utf-8").write( s_resp.content )
    _ctrls = sorted({ hex(ord(c)) for c in s_resp.content if ord(c) < 0x20 })
    print( f"  CONTROL CHARS present in body: {_ctrls}" )
    try:
