...
> 🔴 **REWRITTEN 2026-08-22 on Mr Radio's review** (`2026.08.22-qa-card-plan-review.md` §3). Two of the three gates below were **not falsifiable as first written** — one fired on its own scaffolding, and the other falsified itself with a count that this very section disqualifies four lines later. He was right on both, and the second is the more embarrassing: the section arguing against count-gates contained one.

The failure mode of registry work is a green suite bought by weakening an assertion — the 2026.08.15 design says so at length, and it earned that paragraph.

### Gate 1 — the endpoint equals the registry ✅ unchanged

`GET /api/v2/agents` returns exactly `REGISTRY`'s commands, class for class.

**Must-fail control**: add a spec to the table; the test goes red until the endpoint carries it.

This is a set-equality **across a real boundary** — an HTTP response against a Python table — not a count derived from the thing it checks. It was the good one and it stays as it was.

### Gate 2 — no hand-written agent list survives in the front end 🔧 fixed

**The defect**: the gate greps `#agent-mode` for `<option value=` — and **Auto-Route is a legitimately hand-written option that must stay.** So the gate fires on its own scaffolding on day one, and the only way to green it is an exemption.

**The fix, and why this one rather than the exemption.** Mr Radio offered two: name Auto-Route in the test with its reason, or *"make Auto-Route a rendered option with a sentinel value so there is nothing to exempt."* **Take the second.** An exemption list is a place a future change can be quietly added to; there is no such place if the gate has no exemptions at all. Auto-Route becomes a rendered option with a sentinel command value (`""` or an explicit `__auto__`), emitted by the same renderer as every other option, and the grep then means exactly what it says: *no `<option>` inside `#agent-mode` is hand-written, full stop.*

**Must-fail control**: paste one hard-coded `<option value="math">` back into `#agent-mode`; red. Restore; green.

### Gate 3 — every user-initiable command is reachable from the UI 🔧 fixed twice

**Defect (a) — it tested the wrong field.** As first written it read *"for each `speakable` command…"*. After ruling 1 the dropdown is governed by **`user_initiable`**, so the gate could be **fully green while the dropdown showed the wrong set** — and the hole is precisely the case the new field was invented for. §5.1 makes that argument and §6 then did the thing it argues against.

**The gate is now a set-equality**, same shape as gate 1, different column: `{ c for c, s in REGISTRY.items() if s.user_initiable }` equals the set of option values rendered in `#agent-mode` (minus the sentinel).

**Defect (b) — its must-fail control was a count.** *"Mark one command `speakable=False` and watch the count fall."* A count over a list derived from the registry cannot fail unless a set-equality would have failed first — this section's own ⚠️ says so. A gate that falsifies itself the forbidden way is not falsified.

**🔴 And the honest part, measured rather than assumed** — Clayton raised it, Mr Radio carried it, and **I reproduced it myself** on Sam's branch `wt-qa-card-impl-8407333d` at `fc5cd050` before writing this:
...
| `speakable` | **18** |
| `user_initiable` | **16** |
| `user_initiable − speakable` | **EMPTY** |
| `speakable − user_initiable` | `agent router go to automatic`, `none` — one CONTROL, one the no-command outcome; neither is an agent |

**So the command gate 3 exists to catch does not exist in the data yet.** Today a gate written on `user_initiable` and one written on `speakable` are green on identical inputs; revert the fix and nothing goes red. **That does not retire the fix — it changes what has to carry it.**

⇒ **Gate 3's must-fail control MUST MANUFACTURE THE CASE.** Drive the shared predicate with a synthetic spec — `user_initiable=True, speakable=False` — and assert the dropdown carries it. Revert the gate to `speakable` and *that* control goes red. **That control is the only thing making the rewrite mean anything**, and a builder who skips it ships a gate that asserts nothing while looking correct.

Recorded as *"real but not yet reachable"* rather than quietly rewritten, because that is a different instruction to a builder than *"live defect."*

### ⚠️ Still not a gate
"The dropdown has 16 options." A count over a list derived from the registry cannot fail unless a set-equality would have failed first. Ruled 2026-08-15; the ruling stands, and gate 3(b) above is what happens when it is forgotten inside the section that states it.
