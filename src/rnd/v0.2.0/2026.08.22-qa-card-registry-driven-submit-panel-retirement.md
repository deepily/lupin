...
flowchart TD
    ...
    G -->|receptionist| R{"response.route_reason"}
    R -->|user_picked_receptionist| P["render the receptionist ANSWER<br/>— this is what was asked for"]
    R -->|anything else| J["render the fallback<br/>— we could not route you"]
    I --> K["POST /api/v2/resume<br/>{ pending_id, answer }"]
    K --> G
```

🔴 **THE `receptionist` ARM BRANCHES AGAIN, ON `route_reason` — DO NOT COLLAPSE IT** (row
c242166d). This flowchart branched on `response.path` alone, and `path` cannot answer the
question the card needs answered. Two different outcomes arrive as `path="receptionist"`: a
user who **picked** the receptionist from the dropdown, and a request nothing could route.
Rendering both as "the fallback" tells a user who deliberately asked for the receptionist
...
`route_reason` is the field whose whole job is naming which rule fired (`flow.py:783`). The
marker landed at `ced053a1` (row 1568269d), so the flow can already tell them apart; this
chart was one of the two readers that could not.

⚠️ **This is the version to build from. Phase 3's card does not exist yet** — nothing in
`src/lupin_app/static/js/` renders on `response.path` for the receptionist today
(`arg-interview.js` reads `path` only for `needs_input`). That is why this correction is a
document fix rather than a code fix: it has to land *before* somebody implements the chart
as it was drawn, not after.

**And the gate that can go red** (the recommendation doc's proof obligation): post the
receptionist command deliberately and assert `route_reason == "user_picked_receptionist"`;
post a garbage command and assert `"unknown_command"`. Collapse the two branches back into
one and both assertions cannot hold at once. Those live in
`src/tests/unit/test_v2_flow.py` for the flow side and
`src/tests/unit/test_v2_eval_receptionist_reader.py` for the harness side.
...
| **4** | The `needs_input` inline answer box in the Q&A card | **two claims, each held to its own evidence — see §7a.** **A**: the card carries an argument interview to completion (`:7999`). **B**: the queue then drains the job (`:8000`, sequenced behind row `7451bebe`) |
...
### 7a. Phase 4's gate is TWO claims, not one — re-worded on Mr Radio's ruling, 2026-08-22

The cell used to read *"an agentic job completes end to end from the Q&A card alone."* That
packs two claims into one sentence, and only one of them is phase 4's:

| | claim | whose | venue |
|---|---|---|---|
| **A** | the Q&A card carries an argument interview to completion and hands over a fully-argued command | **phase 4's** | `:7999`, seconds |
| **B** | the queue then drains that job | **CJ Flow's**, owed on another row | `:8000` |

**Claim A needs no agentic job at all.** `agent router go to weather` declares
`required_args=("location",)` and is served inline by `resolve()` — no queue, no pool, no
monopolizer — so `ask → needs_input → resume → answer` exercises the whole phase-4 mechanism
on the venue the AI may run at will.

**Claim B was thought unobservable and is not.** The review's §5 cited `ce29cd20`, which is an
xfail on **`/api/v2/ask`** — a door with no `parent_id_hash` field. Phase 4's agentic path uses
**`/api/v2/submit`**, which has one, and the consumer's Gate B admits a monopolizer's own
lineage children *through* the intake hold. ⚠️ This does **not** retire `ce29cd20`; the ask
door still cannot carry the tag.

🔴 **B is SEQUENCED BEHIND row `7451bebe`**, which owed the live proof that Gate B *admits*
rather than that the tag is merely *threaded* — a distinction that row's own history draws
twice. Full working, including a discrepancy on that row's status that somebody who owns it
should settle: **`src/rnd/v0.2.0/2026.08.22-qa-card-phase-4-observation-method.md`**.
