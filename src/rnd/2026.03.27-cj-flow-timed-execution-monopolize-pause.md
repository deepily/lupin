...
**Status**: IN PROGRESS — Phases 0-4 DONE, Phases 5-6 pending
**Session**: 381
**Date**: 2026-03-27
...
- [ ] **3.7** `src/docs/websocket-events.md` — add `job_paused`, `job_resumed` event entries
...
- [ ] **3.10** Smoke test agentic endpoint with `scheduled_at` (requires live server)

### Phase 4: Unit Tests — DONE

- [x] **4.1** `test_timed_execution.py` — 15 tests, all pass
- [x] **4.2** `test_consumer_timed.py` — 10 tests, all pass
- [ ] **4.3** Protocol compliance tests — deferred (protocol smoke test passes)
- [x] **4.4** Full regression: 2338 passed, 0 regressions from our changes

### Phase 5: Notifications UI + WebSocket Integration — PENDING

- [ ] **5.1** Add `"job_paused"`, `"job_resumed"` to JS `subscribed_events` array (~line 2109)
- [ ] **5.2** Add JS event handlers for `job_paused` / `job_resumed` (~line 2245)
- [ ] **5.3** Add paused/scheduled visual states to `renderJobCard()` (~line 6192)
- [ ] **5.4** Add pause/resume toggle button to todo job cards (~line 6250)
- [ ] **5.5** Add CSS for `.status-paused`, `.paused-badge`, `.scheduled-badge`
- [ ] **5.6** Wire pause/resume endpoints to emit WebSocket events

### Phase 6: Documentation + Session End — PENDING

- [ ] **6.1** `src/docs/websocket-events.md` — event docs
- [ ] **6.2** Update history.md, TODO.md, commit
- [ ] **6.3** Manual E2E test (requires live server — next session)
