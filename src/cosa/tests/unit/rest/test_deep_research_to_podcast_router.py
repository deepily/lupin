"""
Unit tests for the retired Deep Research to Podcast router (`cosa.rest.routers.deep_research_to_podcast`).

WHAT THIS FILE USED TO TEST, AND WHY IT DOES NOT ANY MORE. It covered `submit_research_to_podcast` —
...
`/api/deep-research-to-podcast/submit` is a tombstone answering 410 and naming `/api/v2/submit`, so every one of those
...
import cosa.rest.routers.deep_research_to_podcast as mod
from cosa.rest.routers._retired_doors import RETIRED_DOORS, V2_SUBMIT

PATH = "/api/deep-research-to-podcast/submit"
...
class TestTheDoorIsRetired( unittest.TestCase ):
    ...
    def test_the_route_is_mounted_at_exactly_the_path_the_table_names( self ):
        ...
        `/api/deep-research-to-podcast/api/deep-research-to-podcast/submit` — a 404 wearing a tombstone's clothes.
        """
...
class TestTheModuleNoLongerCarriesALiveDoor( unittest.TestCase ):
    ...
    def test_the_handler_only_refuses( self ):
        ...
        with self.assertRaises( HTTPException ) as caught:
            asyncio.run( mod.submit_research_to_podcast() )
        self.assertEqual( caught.exception.status_code, 410 )

    def test_the_job_building_machinery_is_gone_from_this_module( self ):
        for name in ( "create_agentic_job", "user_job_tracker", "get_todo_queue", "DeepResearchToPodcastJob" ):
    ...
    def test_the_request_and_response_models_are_gone( self ):
        ...
        for name in ( "ResearchToPodcastSubmitRequest", "ResearchToPodcastSubmitResponse" ):
