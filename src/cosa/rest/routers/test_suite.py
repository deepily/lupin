...
class TestSuiteSubmitRequest( BaseModel ):
    ...
    dry_run             : bool            = Field( False, description=(
        ...
        "took the slot. And directly against a REAL suite (ts-37b0d57e running, dry_run "
        "ts-644db39b submitted 2s later at queue position 1): the real job held the "
        "monopolize slot at t=2.1s with the dry run sitting in todo behind it, and the dry "
        "run took the slot at t=15.3s. So 'just checking' a submission does not skip the "
        "line — it joins it, and lands whenever the queue reaches it. "
        "(3) WHAT IS STILL NOT MEASURED: the real suite above was a short one. Nobody has "
        "watched a dry run wait behind a LONG suite, though the queue is one FIFO with one "
        "monopolize slot and nothing in it reads job duration — the wait is simply however "
        "long the work ahead of you takes. "
