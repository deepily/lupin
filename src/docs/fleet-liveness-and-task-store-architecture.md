...
**The sibling-gate lesson (2026-07-12, Krishna 🦚 — three fixes, one family).** Every arbiter false-positive class fixed on 2026-07-12 had the same signature: **a correctness gate wired into ONE consumer of a signal but not its siblings.** `96e8e0f7` — the blocked-edge ROSTER leg lacked the store-corroboration the PING leg had (`edge_is_store_backed`, d44b7068). `de193b5a` — the WORKER-subject stuck advisory lacked the bridge-fresh veto the POKE leg (92c7ab1d) and MANAGER-subject advisory (e5e33795) had. `d71eae1d` — the stuck-episode recovery scan consumed caps on `honored` but not on the sibling `idle` beacon. **Detection method (use it to find the fourth, if one exists)**: don't reason about the code — read the journal until the arbiter *contradicts itself on a single poll* (e.g. `edges=1, pings_fired=0, taps_fired=1`; or `arbiter_stuck_bridge_veto` and a "1 stuck/dead" tap for the same persona in the same poll). **Rule for new gates**: when adding a correctness gate to any consumer of a fleet-view signal, enumerate that signal's OTHER consumers (grep the producer's field name) and either wire the gate uniformly or document per-consumer why not. Recovery-outcome membership rule (52b8ed6b): a recovery outcome is a liveness beacon **the session itself emits** — arbiter-side markers never qualify (`fleet_data_model.py`, `RECOVERY_OUTCOMES`).
...
| Design record | `src/rnd/v0.1.8/2026.06.16-store-canonical-task-mgmt-cascade-review.md` *(REMOVED by `b113a3a7`; recover: `git show b113a3a7^:src/rnd/v0.1.8/2026.06.16-store-canonical-task-mgmt-cascade-review.md`)* (review + cutover log) |
