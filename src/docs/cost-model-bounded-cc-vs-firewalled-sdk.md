...
These all ride the CJ Flow agentic pool with `task_type=BOUNDED` (BFE/TFE) or in-process `sdk_query` (Podcast / Presentation / Deep Research); all have been validated without consuming firewalled-key budget. Use them as code-shape references when migrating new agents.
