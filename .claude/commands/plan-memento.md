...
1. **Read the pointer** — `io/mementos/<persona-slug>.md` (worker) or `.claude-memento.md` (self-`/clear`). The pointer **carries the current record's full content**, so a plain `Read` is already correct: **there is nothing to choose, no mtime tiebreak, no candidates to rank.**
   ...
   - 🔴 **DO NOT merge `1593a17e` while the writer stays reverted.** That commit is lupin's Step 2 — it rewrites this step to name a per-persona pointer `.claude-memento-<persona-slug>.md`. 🔴 **Measured 2026-09-02: three of those files ALREADY EXIST in this tree and two of the three are STALE.** `.claude-memento-mr-radio.md` names session `93a8751c` while that persona's newest root record is `0e61abe3`; `.claude-memento-tiberius.md` names `805189bc` against a newest of `d203e90a`; only rachel's is current. All three were written between 21:18 and 22:30 on 2026-08-31 — inside the 82 minutes the per-persona writer (`072ef7e`, landed 21:09, reverted 22:31) was live — and nothing has maintained them since. ⚠️ **This makes Step 2 worse than an absent file, not better.** Its instruction is *prefer the per-persona pointer; if ABSENT fall back to the shared name and check the header*. For those two personas the preferred file is PRESENT, so the fallback never fires — and the header check is written on the fallback branch only. A stale memento that resolves cleanly is exactly the failure this whole row is about: the failure and the success print the same thing. The writer change (planning-is-prompting `072ef7e`) was reverted at `d4f6c29`, and `memento_io.py:649` still returns the shared `.claude-memento.md` for `slot == "root"`. ⚠️ **Its fallback does not rescue it**: that commit tells the reader the fallback retires on their next write, and against the current writer it never does — a permanent fallback documented as a temporary one. Merging Step 2 alone puts lupin into the exact state planning-is-prompting spent 2026-09-01 repairing. Ordering rule, row `8f5dc4df`: **the command lands WITH or AFTER the writer, never before it.**
