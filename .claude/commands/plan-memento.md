...
1. **Read the pointer** — `io/mementos/<persona-slug>.md` (worker) or, for a self-`/clear`, **`.claude-memento-<persona-slug>.md`** and ONLY THEN the shared `.claude-memento.md`. The pointer **carries the current record's full content**, so a plain `Read` is already correct — but WHICH pointer is now a choice, and the two bullets below are that choice.
   ...
   - ⚠️ **THE FALLBACK IS LIVE TODAY AND IS MEANT TO RETIRE — it is step 1 of 2, ruled by Rick 2026-09-02.** Nothing writes `.claude-memento-<persona-slug>.md` yet: the writer half (`memento_io.py`, `pointer_rel_path`, the `slot == "root"` branch) still returns the shared name, so **today every seat takes the fallback and every seat gets the header check**. That is the intended state, not a gap. When the writer lands, seats acquire the per-persona name on their next write and stop reaching the fallback at all. **Until then, do not treat a per-persona pointer as authoritative just because it exists** — three such files were left behind by an earlier 82-minute window, two of them stale by whole sessions, and they were deleted on 2026-09-02 for exactly that reason.
