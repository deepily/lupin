# Plan Review Gate for Lupin Project

**Project**: Lupin
**Prefix**: [LUPIN]
**Version**: 1.0
**Installed**: 2026-05-07 by session 6825e6af via `/plan-install-wizard` (mode=install)
...
6. **MUST run the three passes strictly sequentially — NEVER in parallel**:
   ...
   - If you would have batched passes for wall-clock efficiency: don't. The §3 ordering rationale (canonical workflow) is load-bearing, and the user has explicitly observed parallel execution as a failure mode. (See `feedback_pip_plan_review_is_sequential` in user auto-memory — Phase 4 + 5 of the multiplexer milestone hit this exact antipattern on 2026-05-05.)

7. **MUST update idempotency marker on success**:
...
Standalone REUSE pre-pass for Pattern 3 plans (single-doc `src/rnd/yyyy.mm.dd-slug.md` files) or any pre-doc-creation reuse audit. Achieved via `--from=reuse-only` or by invoking the canonical §4 prompt directly.
...
## Lupin-specific notes

- **Verification venues**: per Lupin CLAUDE.md §TESTING VENUES, REUSE/Fitness/Adversarial findings that recommend test-running should route per the rubric — `:7999` dev for non-destructive AI-discretionary checks, `:8000` test (monopolize-mode, scheduled via `POST /api/test-suite/submit` with user-confirmed `scheduled_at`) for state-mutating or long-running runs. Never side-door inject `:8000` work.
- **CoSA git boundary**: per `feedback_lupin_only_never_cosa`, Pass 1/Pass 2 findings that propose changes inside `src/cosa/` MUST flag the CoSA git boundary — Claude can edit files in `src/cosa/` but never run git commands there. User commits CoSA in a separate context.
- **Auto-memory feedback loop**: Pass 2 (Adversarial) findings that surface a recurring pattern should call out which `~/.claude/projects/<project-slug>/memory/feedback_*.md` entries (if any) the plan would violate, per the user's hard-rule on plan self-audit before ExitPlanMode.
...
/plan-review --doc-set=src/rnd/v0.1.7/2026.05.07-claude-code-bounded-redesign/  # target a specific milestone
...
This slash command is a **reference wrapper** that reads the canonical workflow document on every invocation. The canonical doc at `workflow/plan-review.md` (in `$PLANNING_IS_PROMPTING_ROOT`) is project-agnostic; this wrapper injects Lupin-specific anchor-file paths, verification-venue rules, and CoSA git boundary callouts. Other projects (cosa-voice, etc.) use their own thin wrappers that inject project-specific tagging conventions and verification venues.
