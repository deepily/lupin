...
**Command Format**:
```bash
notify-claude "[LUPIN] MESSAGE" --type=TYPE --priority=PRIORITY
```
...
```bash
notify-claude "[LUPIN] ✅ Session history updated" --type=progress --priority=low
notify-claude "[LUPIN] Ready for commit approval" --type=task --priority=medium
notify-claude "[LUPIN] 🎉 Session wrap-up complete" --type=task --priority=low
```
...
3. **Take Action Based on Severity**:
   ...
   **If ⚠️ WARNING** (≥20k tokens OR breach <7 days):
   ...
   - **Present options**:
     ...
     [2] Archive next session - adds "[LUPIN] Archive history.md" to TODO
   ...
   - **If [1] selected**:
     ...
     * Wait for completion
     * Send notification: `notify-claude "[LUPIN] ✅ History archived" --type=progress --priority=low`
     * Resume session-end workflow (continue to Step 1)

   - **If [2] selected**:
     * Add "[LUPIN] Archive history.md" to TODO list
     * Send notification: `notify-claude "[LUPIN] History archive deferred to next session" --type=task --priority=medium`
     * Resume session-end workflow (continue to Step 1)

   - **If [3] selected**:
     * Log decision
     * Send notification: `notify-claude "[LUPIN] ⚠️ Continuing with large history.md (manual handling)" --type=alert --priority=medium`
     * Resume session-end workflow (continue to Step 1)
   ...
   **If 🚨 CRITICAL** (≥22k tokens OR breach <3 days):
   ...
   - Send urgent notification: `notify-claude "[LUPIN] 🚨 Critical: History archived to prevent limit breach" --type=alert --priority=urgent`
